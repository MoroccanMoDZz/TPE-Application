...
int GoalDspPicture(T_GL_HWIDGET hWindow, char *pcTitle, const char* pcPicture, const ST_IMAGE* pxImage, ulong ulTimeOut, bool bRefresh)
{
	...
	GL_Widget_SetTransformation(hPicture, GL_TRANSFORMATION_FIT_ALL); // Stretched along all directions
...
void fct()
{
	...
	// ********************
    xDrawing = GoalCreateDrawing(mainLayout, GL_ENCODING_UTF8);
	...
	Telium_Ttestall(0,1000);

	lblKO:                                   // None-classified low level error
		GL_Dialog_Message(mainLayout, NULL, "Processing Error", GL_ICON_ERROR, GL_BUTTON_VALID, 5*1000);
		goto lblEnd;
	lblEnd:
		if(xDrawing)
