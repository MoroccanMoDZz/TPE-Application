...
// =======================================
static const ST_IMAGE na_xImage480 =
{
...
static const ST_IMAGE na_xImage240_1 =
{
	{GL_COLOR_BLACK, TRUE, {NULL, GL_FONT_STYLE_NORMAL, GL_SCALE_MEDIUM}, GL_DIRECTION_NONE, {0, 0, 0, 0, GL_COLOR_RED}, {1, 1, 1, 1} },
	{TRUE, {NULL, GL_FONT_STYLE_NORMAL, GL_SCALE_MEDIUM}, GL_DIRECTION_NONE, {0, 0, 0, 0, GL_COLOR_BLACK}, {1, 1, 1, 1} }
};

static const ST_IMAGE na_xImage2_480 =
{
	{GL_COLOR_BLACK, NULL, {"LUCIDA", GL_FONT_STYLE_NORMAL, GL_SCALE_MEDIUM}, GL_DIRECTION_NONE, {0, 0, 0, 0, GL_COLOR_RED}, {100, 0, 100, 100} },
	{TRUE, {NULL, GL_FONT_STYLE_NORMAL, GL_SCALE_MEDIUM}, GL_DIRECTION_NONE, {1, 1, 1, 1, GL_COLOR_BLACK}, {2, 1, 2, 1} }
};

static const ST_IMAGE na_xImage2_240 =
{
...
//	GoalDestroyPicture(&hPicture);
	...
	GL_Widget_SetBackColor(picture, GL_COLOR_WHITE);
...
//	GoalDspPicture(xPicture, "Autorisation en cours", "file://flash/HOST/Autorisation_encours.png", &na_xImage240, GL_TIME_INFINITE, true);
...
uchar   DemAutor8583(uchar* update_param,uchar* update_date)
{
	...
	/*****************************
		...
		hPicture = GoalCreatePicture(APEMV_UI_GoalHandle());                             // Create and clear screen
		GoalDspPicture(hPicture, "Autorisation en cours", "file://flash/HOST/Autorisation_encours.png", &na_xImage480, (GL_TIME_SECOND) * 2, true);
		GoalDestroyPicture(&hPicture);

		memcpy(R_Transac8583.champs38,"ABCD123", 6);
	...
	*****************************/
	...
	//if(FicPara.Etat_TLS==0)		//Kamalssl
	{
		...
		do{
			...
			if(FicPara.g_type_connexion == 'M');//Fermer17status = connect_host_GSM((PC)phone,(PC)site);
			if(FicPara.g_type_connexion == 'R');//Fermer17status = connect_host((PC)phone,(PC)site);
...
//						}
	...
	if	(
		...
		{
			MessageWithoutConfirmation("Demande facture", "file://flash/HOST/Autorisation_encours.png");
		}
	else if(StructInfoPayment.CodeFunction == OR_FACTURE51)
		{
			MessageWithoutConfirmation("Paiement facture", "file://flash/HOST/Autorisation_encours.png");
		}
	else
	{
...
//
//		hPicture = GoalCreatePicture(APEMV_UI_GoalHandle());                             // Create and clear screen
//		GoalDspPicture(hPicture, "Autorisation en cours", "file://flash/HOST/Autorisation_encours.png", &na_xImage480, (GL_TIME_SECOND) * 0, true);
//		GoalDestroyPicture(&hPicture);
...
//	int count = TMT_Retrieve_Clock();
//	char tmp[100];
	emission=Emission(E_Transac8583.e_type_mess,StructInfoPayment.Bit_Map);

//	sprintf(tmp, "Elapsed Time Emission : %d", TMT_Retrieve_Clock() - count);
//    Os__xprinta(tmp);
    ...
    //gestion des thread demande autorisation et pubilicité
...
//	counter = TMT_Retrieve_Clock();
	FLAG_Ref_SSL = 2;
	reception=Reception();
//	sprintf(tmp, "Elapsed Time Reception : %d", TMT_Retrieve_Clock() - count);
//	Os__xprinta(tmp);
...
//		sprintf(xtime, "%2ld.%03ld Sec", 100 - TimerGet(0) / 1000,1 - (TimerGet(0) % 1000));
//		Os__xprinta(xtime);
//		TimerStop(0);
		return reception;
	}
...
int AnnulationAutomatique()
{
	...
	if(FicPara.AutoReversalOK == 2)
		return 0;
	if(StructInfoPayment.CodeFunction == OR_FACTURE51)
		return 0;
