...
T_Transaction_File Transac_File;
T_Transaction_File Transac_File_Backup;
...
//===========================================================================
static unsigned short crc16_ccitt(const void *buff, int len)
{
					...
					if (nb_elt_read == 1)
					{
						if (T_length > 0)
						{
							...
							if (nb_elt_read == 1)
							{
								...
								crc16_calculated = crc16_ccitt(puc_buffer, T_length);
...
int Add_Transaction(T_Transaction_File *pt_Tr_File, TLV_TREE_NODE hInputTlvTree)
{
	...
	if(hInputTlvTree != NULL)
	{
		...
		if (pt_Tr_File->hFile != NULL)
		{
			...
			if(First_trans)
			{
				crc16_trans = crc16_ccitt(&puc_begin_buf[TRANS_FILE_HEADER_SIZE + INT_LEN], Trans_len);
			}
			else
			{
				crc16_trans = crc16_ccitt(&puc_begin_buf[INT_LEN], Trans_len);
...
//===========================================================================
static int Fill_TLV_Tree(unsigned char * tr_buf, int tr_len, TLV_TREE_NODE *phOutputTlvTree)
{
	int len_read=0;
	DataElement elt;
	int ret = 1;
...
//===========================================================================
int Get_Transaction(T_Transaction_File *pt_Tr_File, TLV_TREE_NODE *phOutputTlvTree, unsigned int Trans_pos, int * ptr_Trans_len)
{
	int ret_exist, ret;
			...
			{	
				...
				if (nb_elt_read == 1)
				{
					if (*ptr_Trans_len > 0)
					{
						...
						{
							// calculate CRC16 on data read
							crc16_calculated = crc16_ccitt(puc_buffer, *ptr_Trans_len);
							...
							if (nb_elt_read == 1)
							{
								...
								{
									if (Fill_TLV_Tree(puc_buffer, *ptr_Trans_len, phOutputTlvTree))
									{
										ret = TRANS_FILE_OK;
									}
								}
...
int Get_Next_Transaction(T_Transaction_File *pt_Tr_File, TLV_TREE_NODE *phOutputTlvTree, unsigned int *pt_trans_pos)
{
	...
	ret = Get_Transaction(pt_Tr_File, phOutputTlvTree, *pt_trans_pos, &T_length);
