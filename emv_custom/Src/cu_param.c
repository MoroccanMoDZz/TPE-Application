...
#ifdef	__TELIUM3__
#include <OSL_Package.h>
#include <OSL_Object.h>
#endif
#ifdef _USE_UCM_
#include "EmvTool.h"
#endif /* _USE_UCM_ */
...
#ifdef __TELIUM3__
#define EMVDC_T3_APPLI_TYPE	(0x03E7B818)
#define EMVDC_APPLI_TYPE	(0x57A4)
#else
...
static const unsigned char C_OLD_FILES[C_NB_OLD_FILES][13] = {C_AID1_PARAM,
C_AID2_PARAM,
C_AID3_PARAM,
C_AID4_PARAM,
C_AID5_PARAM,
...
static void Compute_Cks (CKS_Checksum_t *checksum);

#ifdef __TELIUM3__
static int isKernelServiceT3(void);
#endif
...
int PAR_InitParam(unsigned char uc_x_InitType, char *pc_x_DirFile)
{
	...
	switch (uc_x_InitType)
	{
		...
		case C_PARAM_DEFAULT:
			...
			TlvTree_ReleaseEx(&pTreeDefault);
...
int CUPAR_ControlCAKEYS (void)
{
	...
	do{
		...
		pShaDigest = GTL_SHA_Sha1Compute ((unsigned char*)ac_l_sha_buf, LG_RID+1+key.exponentlength + key.length);
		...
		if ((cr) && (data.elt.length == 20) && (pShaDigest != NULL))
		{
			...
			if (memcmp (ac_l_szReadSha,pShaDigest, 20) != 0 )
			{
				...
				CUPRINT_String_XT((const unsigned char*)ac_l_szTrace, ALIGN_LEFT, L1);
	...
	*nb_aid = 0;
	...
	do{
		...
		if (i_l_cr)
		{
			...
			tab_AID [*nb_aid].no_Aid = i_l_ResultMark;
...
void CUPAR_RestoreParam (unsigned char uc_x_FileMgtType)
{
		...
		(uc_x_FileMgtType == C_MGT_TXT_PAR_FILES))
	{
		...
		if (strlen(InBuf) > strlen(FilePath_L))
		{
			...
			Build_Parameters(C_WORK_FILES_TO_BIN, InBuf, OutBuf, rStringTmp, option);
		}
	}
	...
	{
		Mode = FS_READMOD;
		rc   = FS_mount ((char*)FilePath_L, (unsigned int *)&Mode);
		if (rc != FS_OK)
		{
	...
	/*sprintf(FilePath_L, "/%s/%s", LabelDiskName, "AID1_005D"");
...
void CUPAR_FileCopy(char *DirFile, char *pc_x_fileName)
{
	...
	}
	{
		...
		if (file1 == NULL)
		{
			sprintf((char*)uc_l_trace, "FS_open(%s) ko\n", pc_x_fileName);
...
static int CUPAR_Compute_ICS_Config_CRC(void)
{
	...
	CKS_Checksum_t checksum;
	int Index,i;


	Compute_Cks ( &checksum );

	memclr (value,sizeof(value));
	for (Index = 0, i = 0 ; Index < 20; Index=Index+5, i = i+ 1 )
	     sprintf((char*)&(value[2*Index+i]), " %02X%02X%02X%02X%02X", (int)checksum[Index],(int)checksum[Index+1],(int)checksum[Index+2],(int)checksum[Index+3],(int)checksum[Index+4]);

	cust_full_crc_config_file.elt.length = 45;
	cust_full_crc_config_file.elt.ptValue = value;
	cust_full_crc_config_file.elt.tag = TAG_CUST_FULL_CRC_CONFIG_FILE;

	PAR_SetParameter(TAG_CUST_FULL_CRC_CONFIG_FILE, cust_full_crc_config_file);
...
static int Update_Parameters_file(TLV_TREE_NODE pTree, char * ptr_TxtBuf, char * ptr_OutBuf, char * ptr_rStringTmp, unsigned char option)
{
	...
	else if ((strstr(ptr_TxtBuf, "AID6") != NULL) || (strstr(ptr_TxtBuf, "aid6") != NULL))
	{
		i_l_Mark = C_TAG_AID6;
		strcpy(ac_l_Mark, C_MARK_AID6);
	}
	else if ((strstr(ptr_TxtBuf, "AID7") != NULL) || (strstr(ptr_TxtBuf, "aid7") != NULL))
	{
		i_l_Mark = C_TAG_AID7;
		strcpy(ac_l_Mark, C_MARK_AID7);
	}
	else if ((strstr(ptr_TxtBuf, "AID8") != NULL) || (strstr(ptr_TxtBuf, "aid8") != NULL))
	{
		i_l_Mark = C_TAG_AID8;
		strcpy(ac_l_Mark, C_MARK_AID8);
	}
	else if ((strstr(ptr_TxtBuf, "AID9") != NULL) || (strstr(ptr_TxtBuf, "aid9") != NULL))
	{
		i_l_Mark = C_TAG_AID9;
		strcpy(ac_l_Mark, C_MARK_AID9);
	}
	else if ((strstr(ptr_TxtBuf, "AIDA") != NULL) || (strstr(ptr_TxtBuf, "aidA") != NULL))
	{
		i_l_Mark = C_TAG_AIDA;
		strcpy(ac_l_Mark, C_MARK_AIDA);
	}
	else if ((strstr(ptr_TxtBuf, "AIDB") != NULL) || (strstr(ptr_TxtBuf, "aidB") != NULL))
	{
		i_l_Mark = C_TAG_AIDB;
		strcpy(ac_l_Mark, C_MARK_AIDB);
	}
	else if ((strstr(ptr_TxtBuf, "AIDC") != NULL) || (strstr(ptr_TxtBuf, "aidC") != NULL))
	{
		i_l_Mark = C_TAG_AIDC;
		strcpy(ac_l_Mark, C_MARK_AIDC);
	}
	else if ((strstr(ptr_TxtBuf, "AIDD") != NULL) || (strstr(ptr_TxtBuf, "aidD") != NULL))
	{
		i_l_Mark = C_TAG_AIDD;
		strcpy(ac_l_Mark, C_MARK_AIDD);
	}
	else if ((strstr(ptr_TxtBuf, "AIDE") != NULL) || (strstr(ptr_TxtBuf, "aidE") != NULL))
	{
		i_l_Mark = C_TAG_AIDE;
		strcpy(ac_l_Mark, C_MARK_AIDE);
	}
...
static int isKernelServiceT3(void)
{
	...
	if (isT3 < 0)
	{
		T_OSL_HPACKAGE_INFO packageInfo;

		packageInfo = OSL_PackageInfo_Open(OSL_PACKAGE_TYPE_LEGACY_APPLICATION, 0x03E7AECF);
		if (packageInfo != NULL)
		{
			isT3 = (OSL_PackageInfo_GetRange(packageInfo) != 0);
			OSL_PackageInfo_Close(packageInfo);
			packageInfo = NULL;
		}
	}
...
int CUPAR_Update_Kernel_and_System_Infos (void)
{
	...
	int           i;
	int           iMaxAppli;
	CKS_ChecksumKernel_t kernelchecksum;

	memclr (ct_KernelVersion ,sizeof(ct_KernelVersion));
	ct_KernelCRC =        0;
	ct_SystemVersion =    0;
	ct_SystemCRC =        0;
	...
	for (i = 0; i < iMaxAppli; i ++)
	{
		...
		{
			// Application EMV DC
			if (infos.application_type == EMVDC_APPLI_TYPE)
			{
				memclr (kernelchecksum, sizeof(kernelchecksum));
				if (CKSEMV_KernelChecksum (&kernelchecksum) == CKS_STATUS_SUCCESS)
					 GTL_Convert_BinNumberToUl ((const void*) kernelchecksum, &ct_KernelCRC, 4);

				strcpy (ct_KernelVersion ,infos.name);
...
#else
	T_OSL_HOBJECT_INFO objInfo;
	CKS_ChecksumKernel_t kernelchecksum;

	memset(ct_KernelVersion, 0, sizeof(ct_KernelVersion));
	ct_KernelCRC = 0;

	if (isKernelServiceT3())
	{
		objInfo = OSL_ObjectInfo_Open(OSL_OBJECT_TYPE_APPLI, EMVDC_T3_APPLI_TYPE);
		if (objInfo != NULL)
		{
			OSL_ObjectInfo_GetFamilyName(objInfo, ct_KernelVersion, sizeof(ct_KernelVersion) - 4 - 1);
			sprintf(ct_KernelVersion + strlen(ct_KernelVersion), "%02u%02u", OSL_ObjectInfo_GetVersion(objInfo), OSL_ObjectInfo_GetRelease(objInfo));
			OSL_ObjectInfo_Close(objInfo);
		}
	}
	else
	{
		objInfo = OSL_ObjectInfo_Open(OSL_OBJECT_TYPE_APPLI, EMVDC_APPLI_TYPE);
		if (objInfo != NULL)
		{
			char *ptr;
			OSL_ObjectInfo_GetBinaryName(objInfo, ct_KernelVersion, sizeof(ct_KernelVersion) - 1);
			ptr = strchr(ct_KernelVersion, '.');
			if (ptr != NULL)
				*ptr = '\0';
			OSL_ObjectInfo_Close(objInfo);
		}
	...
	if (CKSEMV_KernelChecksum (&kernelchecksum) == CKS_STATUS_SUCCESS)
		 GTL_Convert_BinNumberToUl ((const void*) kernelchecksum, &ct_KernelCRC, sizeof(kernelchecksum));
#endif
...
void CUPAR_UpdateGlobalParam (void)
{
	...
	if (hTree != NULL)
	{
		strcpy ((char* ) ac_l_chaine, CUMESS_Read_Message(DFT_MESS_NB_APPLI_NAME, merchLang));
		TlvTree_AddChild(hTree, TAG_CUST_APPLI_NAME, ac_l_chaine, strlen ((char *) ac_l_chaine));

		strcpy ((char *) ac_l_chaine, CUMESS_Read_Message(DFT_MESS_NB_IDLE_MESS, merchLang));
...
static void Compute_Cks (CKS_Checksum_t *checksum)
 {
	 TLV_TREE_NODE optionsget;
	 TLV_TREE_NODE node;
	 int found;
	 unsigned long tag;
	 TLV_TREE_NODE options;
	 DataElement elt;

   	 options = TlvTree_New(0);

	 optionsget = TlvTree_New(0);
	 if (optionsget != NULL)
	 {

	 if ( CKSEMV_Engine_GetOptionsList(optionsget) == CKS_STATUS_SUCCESS)
		 {
			...
			node = TlvTree_GetFirstChild(optionsget);
			while(node != NULL)
			{
				 // ici renseigner les tags ........
				tag = TlvTree_GetTag(node);
				found = PAR_ReadParameter(tag, (DataElementExt *)&elt);
				if (found)
					TlvTree_AddChild(options, elt.tag, elt.ptValue, elt.length);

				node = TlvTree_GetNext(node);
			}

			CKSEMV_Engine_ComputeChecksum( options, (CKS_Checksum_t *)checksum );
		 }
	 }

	TlvTree_ReleaseEx(&optionsget);
	TlvTree_ReleaseEx(&options);
 }
