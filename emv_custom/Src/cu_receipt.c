...
/* ======================================================================== */
DataElementExt  cust_full_crc_config_file;
...
void CUPRINT_Open_Printer(void)
{
	...
	if(UCM_Present)
	{
		if (iLIBUCM_Print_Open(UCMC_PRINT,"w-") == FCT_OK)
			g_pPrinter = (Telium_File_t*)1;	// This value may be tested as ok (different from NULL)
	}
	else
#endif
		g_pPrinter = Telium_Fopen("PRINTER", "w-");
...
static int CUPRINT_SetPrintLevel(const t_level level)
{
	...
	{
	case L0 :
		Telium_Pprintf("%c", 0x0F);			// condensed mode
		ticket_width = 48;
	...
	case L1 :
		...
		break;
	case L2 :
		Telium_Pprintf("%cF", 0x1B);			// double height mode
		ticket_width = 24;
		ptr_PrinterMode = _pHEIGHT_;	// warning, this font isn't in the default /LOCAL/MANAGER.SGN file ...
		break;
	case L3 :
		Telium_Pprintf("%c", 0x0E);			// double width mode
		ticket_width = 12;
		ptr_PrinterMode = _dSMALL_;
		break;
	case L4 :
		Telium_Pprintf("%cE", 0x1B);			// bold mode
...
static void CUPRINT_UnsetPrintLevel(const t_level level)
{
	...
	{
	case L0 :
		Telium_Pprintf("%c", 0x12);    // uncondensed mode
		break;
	case L1 :
		break;
	case L2 :
		break;
	case L3 :
		Telium_Pprintf("%c", 0x14);    // undoublewidht mode
		break;
	case L4 :
		Telium_Pprintf("%cF", 0x1B);   // unbold mode
		break;
	default :
		break;
	}
...
void CUPRINT_String_XT(const unsigned char *string, const t_align align, const t_level level)
{
	...
	default :
	case ALIGN_LEFT :
		lenght = strlen((char*)string);
		if (lenght <= ticket_width)
		{
			// Print the string
			Telium_Pprintf("%s", string);

			// TODO a remettre les 2 lignes suvantes ?
			//CurrentMSG.file = ;
		...
		else
		{
			...
			do
			{
				...
				for (; j < i; j ++)
				{
					Telium_Pprintf("%c", string[j]);
				}

				Telium_Pprintf("\n");

				lenght = strlen((char*)&string[i]);
				j = i + 1;
				i = i + ticket_width;
				if (level == L2)
				{
...
static void CUPRINT_Double_String(const unsigned char *left_string, const unsigned char *right_string, const t_level level)
{
	...
	spaces_nb = ticket_width - strlen((char*)left_string) - strlen((char*)right_string);

	// Print left string
	Telium_Pprintf("%s", left_string);

	// Print blanks spaces between the two strings
	for (i = 0; i < spaces_nb-1; i++)
	{
		Telium_Pprintf(" ");
	}

	// Print the right string
	Telium_Pprintf("%s", right_string);
...
static void CUPRINT_Data(const unsigned char *label, const unsigned char *value, const t_level level)
{
	...
	{
		Telium_Pprintf("Too many characters.");
	}
...
static void CUPRINT_Top_Frame(const short offset, const short lenght)
{
	...
	Telium_Pprintf("%c", LEFT_TOP_ROUNDED_CORNER);

	// Print the border
	for (i = 0; i < border_nb; i++)
	{
		Telium_Pprintf("%c", HORIZONTAL_BAR);
	}
...
static void CUPRINT_Top_Frame_String(const short offset, const short lenght, const unsigned char *string, const t_align align)
{
	...
	spaces_nb = spaces_nb - (strlen((char*)string) + 2);
	...
	case ALIGN_LEFT :
		...
		Telium_Pprintf("%c%s%c", HORIZONTAL_END_BAR, string, HORIZONTAL_BEGIN_BAR);
...
static void CUPRINT_String_Frame(const short offset, const short lenght, const unsigned char *string, const t_align align)
{
	...
	default :
	case ALIGN_LEFT :
		string_lenght = strlen((char*)string);
		...
		if (string_lenght <= max)
		{
			...
			Telium_Pprintf("%c", VERTICAL_BAR);

			// Print the string
			for (i = 0; i < string_lenght; i ++)
			{
				Telium_Pprintf("%c", string[i]);
		...
		else
		{
			...
			do
			{
				...
				string_lenght = strlen((char*)&string[ind_begin]);
			}
...
static void CUPRINT_Double_String_Frame(const short offset, const short lenght, const unsigned char *left_string, const unsigned char *right_string)
{
	...
	if ( (short) ((strlen((char*)left_string) + strlen((char*)right_string) + offset + 2)) > (lenght - 2))
	{
	...
	else
	{
		...
		spaces_nb = max - (strlen((char*)left_string) + strlen((char*)right_string));    // Substract the two strings lenght

		// Print the right side bar and the left string
		Telium_Pprintf("%c%s", VERTICAL_BAR, left_string);
...
static void CUPRINT_Bottom_Frame(const short offset, const short lenght)
{
	...
	Telium_Pprintf("%c", LEFT_BOTTOM_ROUNDED_CORNER);
...
void CUPRINT_Boundary_Line(const short offset, const short length)
{
	...
	Telium_Pprintf("%c", HORIZONTAL_BEGIN_BAR);
...
static void card_features(void)
{
	...
	sprintf(buffer, "1 - %s", CUMESS_Read_Message(CU_MESS_CARD_FEATURES, g_language));
	...
	if (select == FALSE)
	{
		sprintf(buffer, "%s%c ", CUMESS_Read_Message(CONSULT_CU_MESS_PSE, g_language), VERTICAL_BAR);
		sprintf(offset, "%c %s", VERTICAL_BAR, CUMESS_Read_Message(CONSULT_CU_MESS_SELECTION_METHOD, g_language));
	...
	if (oda != IS_NOT_SUPPORTED)
	{
			...
			(ulKey_exponent[CERTIFICATE_AUTHORITY] != 0))
		{
			...
			{
				sprintf(buffer, "%lu     %c ", ulKey_exponent[CERTIFICATE_AUTHORITY], VERTICAL_BAR);
			...
			(ulKey_exponent[ISSUER_PUBLIC_KEY] != 0))
		{
			...
			{
				sprintf(buffer, "%lu     %c ", ulKey_exponent[ISSUER_PUBLIC_KEY], VERTICAL_BAR);
				sprintf(offset, "%c - %s", VERTICAL_BAR, CUMESS_ReadMessageReceipt(CU_MESS_KEY_EXPONENT, g_language));
				CUPRINT_Data(offset, buffer, L0);
			}
		}
			...
			(ulKey_exponent[ICC_PUBLIC_KEY] != 0))
		{
			...
			{
				sprintf(buffer, "%lu     %c ", ulKey_exponent[ICC_PUBLIC_KEY], VERTICAL_BAR);
				sprintf(offset, "%c - %s", VERTICAL_BAR, CUMESS_ReadMessageReceipt(CU_MESS_KEY_EXPONENT, g_language));
				CUPRINT_Data(offset, buffer, L0);
			}
		}
							 ...
							 const unsigned long AdditionnalTime)
{
	...
	if (ptr_AdditionnalText)
		...
		{
			sprintf((char *) buffer, "%lu ms%c ", AdditionnalTime, VERTICAL_BAR);
...
static void print_tail(void)
{
	...
	CUPRINT_String_XT((const unsigned char *)CUMESS_Read_Message(CU_MESS_SMO_INFO_0, g_language), ALIGN_CENTER, L1);
	CUPRINT_String_XT((const unsigned char *)CUMESS_Read_Message(CU_MESS_SAGEM_MONETEL, g_language), ALIGN_CENTER, L1);
	CUPRINT_Blanks_Lines(1);
	/*&END FT/08/3956/MA trailer Ingenico */

	CUPRINT_String_XT((const unsigned char *)CUMESS_Read_Message(CU_MESS_SMO_INFO_1, g_language), ALIGN_CENTER, L0);
	CUPRINT_String_XT((const unsigned char *)CUMESS_Read_Message(CU_MESS_SMO_INFO_2, g_language), ALIGN_CENTER, L0);
	CUPRINT_Double_String((const unsigned char*)CUMESS_Read_Message(CU_MESS_SMO_INFO_3, g_language),(const unsigned char*) CUMESS_Read_Message(CU_MESS_SMO_INFO_4, g_language), L0);
...
void CURECEIPT_Print_Ticket_POS_Demo(TLV_TREE_NODE hInputTLVTree, unsigned char status_tr)
{
	...
	{
		strcpy((char *) tmp, CUMESS_Read_Message(DFT_MESS_NB_REFUND_TRANS, lg_code));
	...
	{
		CUPRINT_Top_Frame_String(0, 24, (const unsigned char *)CUMESS_Read_Message(CU_MESS_APPROVED, g_language), ALIGN_CENTER);
	}
	else
	{
		CUPRINT_Top_Frame_String(0, 24, (const unsigned char *)CUMESS_Read_Message(CU_MESS_DECLINED, g_language), ALIGN_CENTER);
	}
	CUPRINT_String_Frame(0, 24, (const unsigned char *)"\0", ALIGN_CENTER);
	...
	CUPRINT_Double_String_Frame(0, 24, (const unsigned char*)CUMESS_Read_Message(CU_MESS_AMOUNT, g_language), buffer);
	CUPRINT_String_Frame(0, 24, (const unsigned char*)" ", ALIGN_LEFT);
	...
	CUPRINT_Double_String_Frame(0, 24, (const unsigned char*)CUMESS_Read_Message(CU_MESS_TRANSACTION_VERIFICATION_STATUS, g_language), tvr_ascii);
	CUPRINT_Bottom_Frame(0, 24);
	...
	{
		CUPRINT_String_Frame(0, 24, (const unsigned char*)" ", ALIGN_LEFT);
		CUPRINT_String_Frame(0, 24, (const unsigned char*)CUMESS_Read_Message(CU_MESS_SIGNATURE, g_language), ALIGN_LEFT);
		CUPRINT_String_Frame(0, 24, (const unsigned char*)" ", ALIGN_LEFT);
		CUPRINT_String_Frame(0, 24, (const unsigned char *)" ", ALIGN_LEFT);
		CUPRINT_String_Frame(0, 24, (const unsigned char*)" ", ALIGN_LEFT);
	}
...
void CURECEIPT_Print_Ticket_Advance_Demo(void)
{
	...
	{
		CUPRINT_Boundary_Line(0, 24);
		CUPRINT_String_XT(CUMESS_Read_Message(CU_MESS_ERROR, g_language), ALIGN_LEFT, L4);
		CUPRINT_String_XT(CUMESS_Read_Message(CU_MESS_KERNEL_NOT_SUPPORT_TOOLWARE, g_language), ALIGN_LEFT, L1);
		CUPRINT_Blanks_Lines(1);

		CUPRINT_String_XT(CUMESS_Read_Message(CU_MESS_EMV_KERNEL, g_language), ALIGN_LEFT, L4);
		sprintf(buffer, "%ld", ct_KernelVersion);
		CUPRINT_Double_String(CUMESS_Read_Message(CU_MESS_VERSION, g_language), buffer, L1);
		sprintf(buffer, "%08lx", ct_KernelCRC);
		CUPRINT_Double_String(CUMESS_Read_Message(CU_MESS_CHECKSUM, g_language), buffer, L1);
		CUPRINT_Boundary_Line(0, 24);
	}
	else
	{
		...
		CUPRINT_String_XT(CUMESS_Read_Message(CU_MESS_ADVANCED_DEMO, g_language), ALIGN_CENTER, L2);
		CUPRINT_Blanks_Lines(2);
...
void CURECEIPT_Print_EMV_Config(void)
{
	...
	if(UCM_Present)
	{
		sprintf (buffer_imp, "%s\n",CUMESS_Read_Message(CU_MESS_EMV_CONFIG_IDENT, g_language));
		Telium_Pprintf (buffer_imp);
	}
	else
#endif
		CUPRINT_String_XT((const unsigned char*)CUMESS_Read_Message(CU_MESS_EMV_CONFIG_IDENT, g_language), ALIGN_CENTER, L2);
	CUPRINT_Blanks_Lines(1);
	...
	CUPRINT_Top_Frame(0, 24);
	CUPRINT_String_Frame(0, 24, (const unsigned char*)CUMESS_Read_Message(CU_MESS_ICS, g_language), ALIGN_CENTER);

	cr = PAR_ReadParameter(TAG_CUST_IS_CONFIG_FILE, &data);
	if (cr)
		CUPRINT_Double_String_Frame(0, 24, (const unsigned char*)CUMESS_Read_Message(CU_MESS_NAME, g_language), data.elt.ptValue );

    // skip line
	CUPRINT_Double_String_Frame(0, 24, (const unsigned char*)"", (const unsigned char*)"");

	// Checksums
	//cr = PAR_ReadParameter(TAG_CUST_FULL_CRC_CONFIG_FILE, &data);

	//if (cr)
		CUPRINT_Double_String_Frame(0, 24, (const unsigned char*)CUMESS_Read_Message(CU_MESS_FULL_CHECKSUM, g_language), cust_full_crc_config_file.elt.ptValue);

	// Skip line
	CUPRINT_Double_String_Frame(0, 24, (const unsigned char*)"", (const unsigned char*)"");

	// EMV Kernel Informations
	CUPRINT_String_Frame(0, 24, (const unsigned char*)CUMESS_Read_Message(CU_MESS_EMV_KERNEL, g_language), ALIGN_CENTER);

	// Kernel Version
	sprintf((char *)buffer, "%s",ct_KernelVersion);
	CUPRINT_Double_String_Frame(0, 24, (const unsigned char*)CUMESS_Read_Message(CU_MESS_VERSION, g_language), buffer);
	// Kernel CRC
	sprintf((char *)buffer, "%08lX", ct_KernelCRC);

	/*&END FT/08/4075/MA print of EMVDC CRC*/
	CUPRINT_Double_String_Frame(0, 24, (const unsigned char*)CUMESS_Read_Message(CU_MESS_CHECKSUM, g_language), buffer);
	...
	if (select_result == 0) /* if no problem */
		while (table_ics_config[i].i_MsgNumber != (int)NULL)
		{
			...
			pc_l_Msg = (unsigned char*)CUMESS_Read_Message(table_ics_config[i].i_MsgNumber, g_language); /* Table number not used */
			sprintf((char*)buffer, "%s:", pc_l_Msg);
