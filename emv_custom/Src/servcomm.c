...
static int __COMM_UseTlvTree(void)
{
	...
	if (ObjectGetDescriptor(OBJECT_TYPE_APPLI, g_usCommAppType, &DcDescriptor) == 0)
	{
		if ((DcDescriptor.file_name[8] == '.') && (strncmp((const char*)DcDescriptor.file_name, "3423", 4) == 0))
		{
			...
			pPtr = (const char*) &DcDescriptor.file_name[4];
										...
										StructProcessStep* pProcessStep, TLV_TREE_NODE* pOutputTree)
{
	...
	if (Telium_ServiceGet(usAppType, usServiceID, &ucPriority) == 0)
	{
		...
		if (bResult)
		{
			...
			bResult = (Telium_ServiceCall(usAppType, usServiceID, sizeof(StructProcessStep), pProcessStep, &nResult) == 0);
...
//===========================================================================
TLV_TREE_NODE COMM_Receive_File_TlvTree(const TLV_TREE_NODE hInputTLVTree)
{
	return __COMM_Service(I_COMM_RECEIVE_FILE, hInputTLVTree);
}
