...
{//Os__xprintaa("3333");
	...
	else
	{
		...
		StructInfoPayment.Entry_Mode = TRS_EMV; //kamal premi√®re remont√©e du champ 23 et 55
...
void EMVCUST_Choose_Language_Menu_TlvTree (TLV_TREE_NODE *phOutputTLVTree)
{
    ...
    if(1) // Toujours la langue franÁaise
    {
		DataElementExt data;
		data.elt.length = 2;
...
//			GoalDestroyPicture(&hPicture);
			if(StructInfoPayment.CodeFunction == PREAUTHCODE)
				MessageWithDelay("PrÈauto acceptÈe", "file://flash/HOST/Approuvee.png", 1000);
			else if (StructInfoPayment.CodeFunction == VOIDCODE)
				MessageWithDelay("Annulation acceptÈe", "file://flash/HOST/Approuvee.png", 1000);
			else
				MessageWithDelay("Transaction acceptÈe", "file://flash/HOST/Approuvee.png", 1000);
