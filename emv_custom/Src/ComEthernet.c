...
char g_szProfile[40] ="STUNNEL";
extern short FLAG_Ref_SSL;
SSL_HANDLE sslHdl;
SSL_PROFILE_HANDLE ProfileHnd;
short FlagSSLPRo=0;

extern int connectionIsGoingOn;
...
void Function_lblComKO(int iRet)
{
	...
	_clrscr();                                                      // Clear screen
	//DisplayMedium(0, 0, LL_ErrorMsg(iRet));                         // Link Layer error
	//APEMV_UI_MessageDisplayDelay (LL_ErrorMsg(iRet), 2);
	if (iRet == LL_Network_GetStatus(LL_PHYSICAL_V_ETHERNET, &iStatus))
	{
		switch(iStatus)
		{
...
int DisconnectSSL()
{
	if(FicPara.Etat_TLS == 1)
	{
		...
		if(sslHdl)
		{
			SSL_Disconnect(sslHdl);
			SSL_Free( sslHdl );
			//Os__xprintaa("DisconnectSSL");
			//sslHdl = NULL;
		}
		//APEMV_UI_MessageDisplayDelay("DisconnectSSL.1",1);
	}
}
