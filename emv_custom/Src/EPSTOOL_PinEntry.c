...
EPSTOOL_PinEntry_Status_e EPSTOOL_PinEntry(T_GL_HGRAPHIC_LIB goalHandle, const char *title, const char *text, const char *help, EPSTOOL_PinEntry_Infos_t *pinEntryInfo, int *pinLength)
{
	...
	if(goalHandle != NULL)
	{
//		GL_GraphicLib_SetBackgroundImage(goalHandle, "file://flash/HOST/whitebg.bmp");
//		GL_GraphicLib_SetSkinedColor(goalHandle, GL_COLOR_DIALOG_TEXT, GL_COLOR_BLACK);
		GL_Dialog_Scheme(goalHandle, title, text, help, &pinEntryInterface);
	}
	else
	{
		// It is a non graphic pinpad
		if(__EPSTOOL_PinEntry_Open(&pinEntryInterface) == GL_RESULT_SUCCESS)
		{
			do
			{
				result = __EPSTOOL_PinEntry_Refresh(&pinEntryInterface, NULL);
			}while(result == GL_RESULT_SUCCESS);

			__EPSTOOL_PinEntry_Close(&pinEntryInterface);
		}
	}
	...
	if (pinLength != NULL)
	{
		*pinLength = pinEntryContext.pinLength;
		...
		//APEMV_UI_MessageDisplayDelayHexa(pinEntryInterface.privateData,pinEntryContext.pinLength,2);
