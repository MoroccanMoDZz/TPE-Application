...
//    - Telium_Fopen(), Telium_Fclose() (enable, disable display)
//    - Telium_Putc(), Telium_Puts(), Telium_Printf() (write character, string and formatted string)
	...
	while(1)
	{
		...
		{                                          // No, it's less than 5 lines with '\n' detected
			...
			else                                   // '\n' detected
			{
				...
				if (*pcData != '\n')               // no more '\n' => print the buffer
				{
					iRet = Telium_Pprintf("%s", tcBuffer);
					CHECK(iRet>=0, lblKO);
					iCount=0;
				}
			}
			  ...
			  int iMaxItems, const char *tpcItems[])
{
	...
	Telium_Ttestall(ENTRY, 0);                              // Wait for confirmation key
...
Telium_File_t *OpenConverter(char *pcName, byte ucSize)
{
	...
	{
		pxCom = Telium_Fopen(tzConverter[i], "rw*");         // Open converter channel
		if(pxCom)
		{
			Telium_Status (pxCom, &ucSta);                   // Check if converter recognized
			if ( (ucSta & USB_PRESENT) == USB_PRESENT)
			{
				...
				Telium_Fclose(pxCom);                        // Close converter unrecognized
		}
	}
...
//****************************************************************************
//                  void BitOn (char *pcByte, byte ucIdx)
//  This function sets on the bit number Idx into Byte. Bits are numbered from
//  right to left starting from 0.
//  This function has parameters.
//     pcByte (IO) : Byte containing a bitmap
//     ucIdx (I-) : Related bit number
//  This function has no return value.
//****************************************************************************

static void BitOn (char *pcByte, byte ucIdx)
{
	*pcByte |= 1 << ucIdx;
}

//****************************************************************************
//                  void BitOff (char *pcByte, byte ucIdx)
//  This function sets off the bit number Idx into Byte. Bits are numbered from
//  right to left starting from 0.
//  This function has parameters.
//     pcByte (IO) : Byte containing a bitmap
//     ucIdx (I-) : Related bit number
...
static void BitOff (char *pcByte, byte ucIdx)
{
	*pcByte &= ~(1 << ucIdx);
}

//****************************************************************************
//               byte BitTest (char cByte, byte ucIdx)
//  This function tests the bit number Idx into Byte. Bits are numbered from
//  right to left starting from 0.
//  This function has parameters.
//     pcByte (I-) : Byte containing a bitmap
//     ucIdx (I-) : Related bit number
//  This function has return value.
...
static byte BitTest (char cByte, byte ucIdx)
{
	return (cByte >> ucIdx) & 0x01;
}

//****************************************************************************
//      void SetParity (char *pcByte, byte ucPar)
//  This function sets the parity bit into Byte (even or odd).
//  This function has parameters.
//     pcByte (IO) : Byte to be set
//     ucPar (I-) : Even or Odd parity
//  This function has no return value.
//****************************************************************************
...
void SetParity (char *pcByte, byte ucPar)
{
	...
	case 'E':
		if (ucCnt%2)
			BitOn(pcByte, 0); // An even parity bit is set to 1 if the number of ones in a given set of bits is odd
		else
			BitOff(pcByte, 0);
		break;
	case 'O':
		if (!(ucCnt%2))
			BitOn(pcByte, 0); // An odd parity bit is set to 1 if the number of ones in a given set of bits is even
		else
			BitOff(pcByte, 0);
		break;
...
//****************************************************************************
//    byte CheckParity (char cByte, byte ucPar)
//  This function checks the parity bit into Byte (even or odd).
//  This function has parameters.
//     cByte (I-) : Byte to be checked
//     ucPar (I-) : Even or Odd parity
//  This function has return value.
//    1 : if parity is OK
//    0 : if parity NOK
