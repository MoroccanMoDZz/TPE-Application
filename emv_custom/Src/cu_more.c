...
int Ava_pourboire(void){                 //larbi
	...
	if(Os__xget_key(FicPara.TimeOut*6) != KEY_CLEAR)
		...
		InitDefaut(_ON_, 1);
		StateHeader(0);
		switch (GL_Dialog_Menu(hGoal, "Sélectionner le pourboire :", MenuPourboire, 0, GL_BUTTON_DEFAULT, GL_KEY_1, GL_TIME_MINUTE)) {
		case 0:
			CinqP_pourboire();
			break;
		case 1:
			...
			if (TransactionLoopMode == TRUE)
			{
                ...
                while (1)
				{
					...
					case ERR_ENG_OK:
						break;
					default:
						EchoMessage_ppad(7);// XEROX  : For display on DDT screen , i send you :	011	006	TM,105	Problème lecture carte, Veuillez retirer et insérer à nouveau votre carte bancaire
...
//					Os__xprintd(AlternanceIsOn);
				...
				else						//MAG
				{
					...
					EchoMessage_ppad(5);// XEROX  : For display on DDT screen , i send you :	011	006	TM,105	Problème lecture carte, Veuillez retirer et insérer à nouveau votre carte bancaire
					Telium_Ttestall(0, 100);
			...
			else if((!memcmp(R_Transac8583.champs39,"000",3)) && (*p_status_tr == 0))
			{
				...
				else
				{
					...
					//devoir controler les données de TM002 avec TM101
					//Si KO Remplir CRPIN par code # de 000 et sortir par break et en génère une annulation
