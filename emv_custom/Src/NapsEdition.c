...
////
	...
	CHECK(iRet >= 0, lblKO);
	xLine.xFont.eScale = GL_SCALE_LARGE;
	sprintf(pPtr, "Conservez-moi, je peux être utile !");
	iRet = GoalPrnLine(xDocument, p++, pPtr, &xLine);
	CHECK(iRet >= 0, lblKO);
...
/*-----------------------------------------------------------------
 * Fonction   :Imprime_ticket_teleechou
 * objet      :probleme de connectivité
 *
 * Parametre  :
 * Retour     :
 *-----------------------------------------------------------------
...
void Imprime_ticket_trarefus()
{
	...
	sprintf(pPtr, "Conservez-moi, jê peux être utile !");
	iRet = GoalPrnLine(xDocument, p++, pPtr, &xLine);
	CHECK(iRet >= 0, lblKO);
...
//		xLine.xFont.eScale = GL_SCALE_XLARGE;
	...
	if(!strcmp(StructInfoPayment.OR_invoiceNumber[j]," "))
		...
		iRet = GoalPrnLine(hDocument, p++, "Montant Payé", &xLine);
