...
//			StructInfoPayment.Transaction_Amount = l_ul_montant;
		...
		case	PREAUTHCOMPCODE:
			//StructInfoPayment.Flag_Dem_Aut=1;     //larbi 10042019 décommenter instruction
			break;
...
//	}
	...
	else
	{
		...
		case OK :
			...
			MessageWithDelay("Annulation acceptée", "file://flash/HOST/Approuvee.png", 1000); // jrizki 200319
...
/*	ShowTrace_Mouna("ad8");
	...
	/*****/
 	...
 	else if(transac.Entry_Mode == TRS_SCT)
 	{
 		...
 		//for(j=0;((j<20)&&(iso2[j] !='d')&&(iso2[j] !='D')&&(iso2[j] !='='));j++);
 		for(j=0;((j<19)&&(iso2[j] !='d')&&(iso2[j] !='D')&&(iso2[j] !='='));j++);//badr problème d'annulation des transaction avec PAN CARD = 19
		memcpy(&StructInfoPayment.Holder_Card[1],iso2,j);
		StructInfoPayment.Holder_Card[0] = j;
		if(j==19)
			memcpy(StructInfoPayment.SeviceCode,transac.SeviceCode,3);
...
/*	ShowTrace_Mouna("ad10");
	...
	else if(StrPreAut.Entry_Mode == TRS_SCT)
	{
		...
		//for(j=0;((j<20)&&(iso2[j] !='d')&&(iso2[j] !='D')&&(iso2[j] !='='));j++);
 		for(j=0;((j<19)&&(iso2[j] !='d')&&(iso2[j] !='D')&&(iso2[j] !='='));j++);//badr problème PAN CARD = 19
		memcpy(&StructInfoPayment.Holder_Card[1],iso2,j);
		StructInfoPayment.Holder_Card[0] = j;
		if(j==19)
...
*/
	...
	if (FicPara.Ctrl_fraude != '1') {
		ShowTrace("VD2");
		//return OK;//enlevé par Hind SAHEL pour demander les 4 derniers chiffres
	}
	if (StructInfoPayment.Entry_Mode == TRS_MAN) {
		//		ShowTrace("VD3");
		return OK ;
...
//	StructInfoPayment.avis=StrLineTra.avis;                          //
	...
	switch(l_retour)
	{
		case OK :
			...
			MessageWithDelay("Annulation acceptée", "file://flash/HOST/Approuvee.png", 1000); // 200319 jrizki
...
void PurgePreautoNaps(void)                                  //1.11.61
{
	...
	for (i = 0, j = 0; (i < Nb_preauto); i++) {
		...
		if (StrPreAut.CodeFunction == VOID_PREAUTO_TOTAL_CODE) {
			//sprintf(tracepurge,"ANN TOT : %s effacé",StrPreAut.STAN);
		} else if (StrPreAut.CodeFunction == VOID_PREAUTO_PARTIEL_CODE) {
			//sprintf(tracepurge,"ANN PAR : %s effacé",StrPreAut.STAN);
		} else if (StrPreAut.StatusAnnulAdjust == 1) {
			//sprintf(tracepurge,"PRE ANN : %s effacé",StrPreAut.STAN);
		} else {
			write_preauto_index(j, &StrPreAut);
			j += 1;

			//sprintf(tracepurge,"PRE AUT : %s gardé",StrPreAut.STAN);
		}
