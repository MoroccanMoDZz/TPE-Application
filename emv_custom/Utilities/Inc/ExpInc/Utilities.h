...
typedef enum eTextSize { ///<Printer Text Scales
    textSizeXXSmall = 0x3FF0, ///< Extra extra small font size (Optimized to display an average of 24 characters on the screen width)
    textSizeXSmall = 0x3FF1, ///< Extra small font size  (Optimized to display an average of 24 characters on the screen width)
    textSizeSmall = 0x3FF2, ///< Small font size (Optimized to display an average of 22 characters on the screen width)
    textSizeMedium = 0x3FF3, ///< Medium font size (Optimized to display an average of 20 characters on the screen width)
    textSizeLarge = 0x3FF4, ///< Large font size (Optimized to display an average of 16 characters on the screen width)
    textSizeXLarge = 0x3FF5, ///< Extra large font size (Optimized to display an average of 12 characters on the screen width)
    textSizeXXLarge =0x3FF6 ///< Extra extra large font size (Optimized to display an average of 10 characters on the screen width)
} eTextSize;

typedef enum eTextAlign { ///<Printer Text Scales
    textAlignCenter = 0x00, ///< the item is placed in the center
    textAlignLeft = 0x01, ///< the item is placed in horizontal on the left
    textAlignRight = 0x02, ///< the item is placed in horizontal on the right
    textAlignTop = 0x04, ///< the item is placed in vertical on the top
    textAlignBottom = 0x08, ///< the item is placed in vertical on the bottom
} eTextAlign;
 ...
 * \param size (I) Size of the string.
 * \return
 *    - negative if failure.
 *    - non negative if OK.
 * \header Utilities.h
 ...
 * \param size (I) Alignment of the string.
 * \return
 *    - negative if failure.
 *    - non negative if OK.
 * \header Utilities.h
...
typedef struct sGoalRange {
    short min;                  //The minimum limit of the scroll bar
    short max;                  //The maximum limit of the scroll bar
    short value;                //The value of the scroll bar
    unsigned short page;        //The page size of the scroll bar (scales the size of slider in the scrollbar)
    unsigned short step;        //The step increment
} tGoalRange;

typedef short (*progressUpdate) (short val);    ///<virtual method function for progress bar
typedef void (*sliderUpdate) (tGoalRange range);    ///<virtual method function for slider

/** Dialog to display image file
 *
 * \param title (I) header to be displayed
 * \param text (I) text to be displayed
 * \param buf (I) location of the file to be displayed
 * \param delay (I) Timeout delay
 *
 * \return
 *  - key entered
 ...
 * \param title (I) header to be displayed
 * \param text (I) text to be displayed
 * \param loc (I) location of the media file to be displayed above the progress bar
 * \param sta (I) virtual function to update the status of the progress bar
 * \param delay (I) Timeout delay
 *
 * \return
 *  - key entered
 ...
 * \param title (I) header to be displayed
 * \param text (I) text to be displayed
 * \param loc (I) location of the media file to be played
 * \param vol (I) volume of the media file to be played (0-255)
 * \param delay (I) Timeout delay
 *
 * \return
 *  - key entered
 ...
 * \param title (I) header to be displayed
 * \param text (I) text to be displayed
 * \param loc (I) location of the media file to be displayed above the slider bar
 * \param val (O) location of the slider
 * \param sta (O) status of the slider 
 * \param delay (I) Timeout delay
 *
 * \return
 *  - key entered
