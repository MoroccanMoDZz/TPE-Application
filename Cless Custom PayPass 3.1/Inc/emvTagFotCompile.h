...
#define TAG_AS_REGISTERED_PROPRIETARY_DATA	0x9F0A				//!< Application Selection Registered Proprietary Data. Variable length (ID on 2 bytes, Length on 1 byte, Value, ...).
#define TAG_PAYMENT_ACCOUNT_REFERENCE		0x9F24				//!< Payment Account Reference (PAR). Format = an (upper case only). Length = 29 bytes.
...
#define TAG_CERT_AUTH_PUB_KEY_MAC               	0x9F81BE 		//!< Certification Authority Public Key MAC. Computed MAC that guarantee the integrity of a Certification Authority PK (CA PK). Length = 4 bytes.
...
//! \def TAG_TR_REACTIVATE_IN_PROGRESS
//! \brief Indicate if a reactivation transaction is running, In that case
//!  Application blocked is ignored, online transaction is forced and Whatever response of generate AC Issuer scripts will be launched
//! Length : 1 byte
//! - TRUE : Reactivation Transaction is running
//! - FALSE : 
#define TAG_TR_REACTIVATE_IN_PROGRESS               0x9F81BB

//! \def TAG_DO_NOT_USE_ICC_PIN_PK
//! \brief Indicate if ICC PIN Encipherment key has not to be used for PIN encryption if present.
//! By default (if this parameter does not exist) behaviour is the same as FALSE this means standard processing.
//! 
//! Length : 1 byte
//! - 0 : Use ICC PIN Encipherment key for PIN encryption if present.
//! - 1 : Do not use ICC PIN Encipherment key. So ICC key will be used. WARNING: It is not EMVCo compliant.
#define  TAG_DO_NOT_USE_ICC_PIN_PK					0x9F81BC

//! \def TAG_ICC_PIN_HASH_INCLUDE_STATIC_DATA
//! \brief Indicate that the ICC PIN certificate hash includes the "Static Data to be Authenticated".
//! By default (if this parameter does not exist) behaviour is the same as FALSE this means standard processing.
//!
//! Length : 1 byte
//! - 0 : ICC PIN certificate hash does not include the "Static Data to be Authenticated", as specified by EMVCo.
//! - 1 : ICC PIN certificate hash includes the "Static Data to be Authenticated". WARNING: It is not EMVCo compliant.
#define  TAG_ICC_PIN_HASH_INCLUDE_STATIC_DATA		0x9F81BD

//! \def TAG_EMV_INT_KERNEL_DOMAIN
//! \brief Indicates the kernel domain to use.
//!	
//! Length : 4 bytes
//! - Example: 00 00 03 E7 for domain 999.
#define TAG_EMV_INT_KERNEL_DOMAIN					0x9F81BF
...
#define KERNEL_DEFAULT_DOMAIN						(999)			//! Default kernel domain.
#define KERNEL_OTHER_DOMAIN							(998)			//! Other possible kernel domain.
...
* \author ------------------------------------------------------------------------------\n
* \author Copyright (c) 2001 - 2016 Ingenico.\n
...
//! \def TAG_CUST_TRREACTIVATE_IS_ASKED
//! \brief Boolean given in response to a function called by menu.
//! Length = 2 bytes. TRUE if Engine must start a transaction card reactivation.
#define TAG_CUST_TRREACTIVATE_IS_ASKED		0x9F842E
...
#define TAG_CUST_DEFAULT_TDOL_TVR_SET_AFTER_1ST_GENAC 	0x9F8468				//!< Tag indicating that TAA sets the default TDOL TVR bit performed BEFORE (  default )  GENERATE AC     new in 4.3f
#define TAG_CUST_NUMBER_OF_SUPPORTED_AIDS				0x9F8469				//!< Tag indicating the supported AIDs default value is 120    new in 4.3f
#define TAG_CUST_IS_RECEIPT								0x9F846A				//!< Tag indicating that Application supports Receipt (by printing or any electronic means )  TRUE ( default )  new in 4.3f
#define TAG_CUST_IS_STORE_DECLINED_TRANSACTIONS			0x9F846B				//!< Tag indicating that application stores declined transactions:  FALSE ( default )  new in 4.3f
