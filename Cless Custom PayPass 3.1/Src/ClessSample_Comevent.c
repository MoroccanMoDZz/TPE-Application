...
void ClessSample_Comevent_HexDump(unsigned char *Header,unsigned char *String,unsigned long Number)
{
	...
	for (i=0;i<Number;i+=6)
	{
		...
		Telium_Pprintf("%s\n",Line);Telium_Ttestall(PRINTER,0);
	}
	Telium_Pprintf("\n\n\n\n\n");Telium_Ttestall(PRINTER,0);
...
static void _ClessSample_Comevent_DisplayData(char *data,unsigned int len)
{
	...
	msg.coding = _ISO8859_;
	msg.file = GetCurrentFont();
	msg.message = buffer;
	buffer[0] = 0;

	HelperDisplayMessageOnTerminalLong(TRUE, 0, &msg);
	...
	for (i=0;i<len;i++)
	{
		...
		{
			buffer[lineLength] = 0;
			HelperDisplayMessageOnTerminalLong(FALSE, line, &msg);
			line++;
			lineLength = 0;
		}
		else
		{
			buffer[lineLength] = data[i];
			lineLength++;
		}
	}

	Helper_RefreshScreen(FALSE, TRUE);
}
...
void ClessSample_Comevent_SendMsg (char * Msg, unsigned short ucEventType)
{
	...
	if (ObjectGetInfo(OBJECT_TYPE_APPLI, ApplicationGetCurrent(), &objInfo) == 0)
	{
		receiver = TaskApplication*256 + (objInfo.application_type & 0x00FF);
		sender   = (objInfo.application_type & 0xFF00) + 0x00;
		...
		strcpy((char*)message.value,(char*)Msg);
...
static void _ClessSample_Comevent_TaskProtocole(void)
{
	...
	Telium_File_t          *handle=NULL; // Decriptor of serial link
	...
	Telium_File_t * hDisplay;			// Descriptor for display
	...
	Telium_SignalEvent(m2os_task,EVT_DIALOG);

	while (1)
	{
		cr = Telium_Ttestall(mask,0);
		...
		case TASK_OPEN:
			...
			if (cr == DIALOG)
			{
				if (param_comin.action==DO_OPEN)
				{
					...
					handle = Telium_Fopen("COM0","rw");

					// initialize peripheral
					// 9600 bauds,8 bits, 1 stop bit,no parity
					Telium_Format("COM0",9600,8,1,NO_PARITY,EVEN,0);
		...
		case TASK_READING :
			...
			if (cr == DIALOG)
			{
				if (param_comin.action==DO_RECEPT)
				{
					...
					do
					{
						nb=Telium_Fread(buf,1,128,handle);
						if (nb)
						{
							if (len<sizeof(data))
							{
					...
					while(Telium_Ttestall(COM0,200)==COM0);
...
void ClessSample_Comevent_Start(NO_SEGMENT no)
{
	...
	m2os_task = Telium_CurrentTask();
  
	// Start Protocole Task
	pttask = (t_topstack *) Telium_Fork((PFONCFORK)_ClessSample_Comevent_TaskProtocole,NULL,0);
	GTL_Traces_TraceDebug("StartComEvent %x", Telium_GiveNoTask(pttask));

	// Waiting for task to be started
	Telium_Ttestall(DIALOG,0);
	if (ObjectGetInfo(OBJECT_TYPE_APPLI, ApplicationGetCurrent(), &objInfo) == 0)
		GTL_Traces_TraceDebug("started !! %x", objInfo.application_type);
	else
...
int ClessSample_Comevent_Event (NO_SEGMENT noappli, S_COMIN *param_in, S_COMOUT *param_out)
{
	...
	case DO_CLOSE :
		...
		Telium_SignalEvent(Telium_GiveNoTask(pttask),EVT_DIALOG);
