	...
	case (CLESS_SAMPLE_SCREEN_PRESENT_CARD):
		...
		if (bAmountCanBeDisplayed)
		{
			...
			if (CS_GlobalTransactionData_GetAmountAuthorized(&ulAmountTemp))
			{
				...
				if (CS_GlobalTransactionData_GetCurrencyLabel(&pCurrencyLabel))
				{
					...
					///ClessSample_Common_FormatAmount(ucFormat, ClessSample_Fill_GiveMoneyLabel(), ucPosition, pTransactionData->ulTransactionAmount, (char*)ucFormattedAmountMessage);
					ClessSample_Common_FormatAmount(ucFormat, pCurrencyLabel, ucPosition, ulAmountTemp, (char*)ucFormattedAmountMessage);
		...
		if (ClessSample_Goal_IsAvailable())
		{
			if ((bDisplayClessLogoInternal) && (nInstructionLineCardholder > nAmountLineCardholder))
			{
				for (nIndex=(nAmountLineCardholder+1); nIndex<nInstructionLineCardholder; nIndex++)
					ClessSample_Goal_SetBlankLine (nIndex, NOCLEAR);
			}			
		}
		...
		if (bDisplayClessLogoInternal)
		{
			if (!ClessSample_Goal_IsAvailable())
				...
				goalTarget = TM_DisplayClessTarget(ClessSample_Goal_Handle());
		}
...
//					Helper_DisplayTextMerchant(NO_ERASE, HELPERS_MERCHANT_LINE_3, &tMsg, NOLEDSOFF);
					//APEMV_UI_MessageDisplayDelay("RETIRER\nCARTE OU MOBILE", 2);
					MessageWithWaitForAction("RETIRER\nCARTE OU MOBILE", NULL, 2000);
					vg_remove = 1;
				}
//				Helper_RefreshScreen(NOWAIT, HELPERS_MERCHANT_SCREEN);
			}
...
//			Helper_RefreshScreen(NOWAIT, HELPERS_CUSTOMER_SCREEN);
		...
		else
		{
			if (ulScreenIdentifier != CLESS_SAMPLE_PAYPASS_SCREEN_REMOVE)
			{
				if (auCustomerDisplayAvailable) // If a customer display is available
				{
					ClessSample_Term_Read_Message(VISAWAVE_MESS_REMOVE_CARD, nMerchantLanguage, &tMsg);
					Helper_DisplayTextMerchant(ERASE, HELPERS_MERCHANT_LINE_2, &tMsg, NOLEDSOFF);
				...
				// Display the customer message
				ClessSample_Term_Read_Message(VISAWAVE_MESS_REMOVE_CARD, nCardholderLanguage, &tMsg);
				Helper_DisplayTextCustomer (ERASE, HELPERS_CUSTOMER_LINE_2, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
			...
			else // if (ulScreenIdentifier == CLESS_SAMPLE_PAYPASS_SCREEN_REMOVE)
			{
				if (auCustomerDisplayAvailable) // If a customer display is available
				{
					ClessSample_Term_Read_Message(PAYPASS_MESS_REMOVE, nMerchantLanguage, &tMsg);
					Helper_DisplayTextMerchant(ERASE, HELPERS_MERCHANT_LINE_2, &tMsg, NOLEDSOFF);
					ClessSample_Term_Read_Message(PAYPASS_MESS_REMOVE_L2, nMerchantLanguage, &tMsg);
					Helper_DisplayTextMerchant(NO_ERASE, HELPERS_MERCHANT_LINE_3, &tMsg, NOLEDSOFF);
					Helper_RefreshScreen(NOWAIT, HELPERS_MERCHANT_SCREEN);
				}

				// Display the customer message
				ClessSample_Term_Read_Message(PAYPASS_MESS_REMOVE, nCardholderLanguage, &tMsg);
				Helper_DisplayTextCustomer (ERASE, HELPERS_CUSTOMER_LINE_2, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
				ClessSample_Term_Read_Message(PAYPASS_MESS_REMOVE_L2, nCardholderLanguage, &tMsg);
				Helper_DisplayTextCustomer (NO_ERASE, HELPERS_CUSTOMER_LINE_3, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
				Helper_RefreshScreen(NOWAIT, HELPERS_CUSTOMER_SCREEN);
			}
		}
...
//		Helper_DisplayTextCustomer (ERASE, HELPERS_CUSTOMER_LINE_2, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
//		Helper_RefreshScreen(NOWAIT, HELPERS_CUSTOMER_SCREEN);
		MessageWithDelay(tMsg.message, 2);
		break;
...
//			Os__xprinta("Trace 4");
			...
			Helper_DisplayTextMerchant(ERASE, HELPERS_MERCHANT_LINE_3, &tMsg, NOLEDSOFF);
			Helper_RefreshScreen(NOWAIT, HELPERS_MERCHANT_SCREEN);
//			MessageWithoutConfirmation("AUTORISATION KO", NULL);
		}

//		Os__xprinta("Trace 4.2");
...
//////////////////////////////
    ...
    case (CLESS_SAMPLE_PAYWAVE_SCREEN_STOPPED):
		ClessSample_Term_Read_Message(STD_MESS_STOPPED, nMerchantLanguage, &tMsg);
		Helper_DisplayTextMerchant(ERASE, HELPERS_MERCHANT_LINE_3, &tMsg, NOLEDSOFF);
		Helper_RefreshScreen(NOWAIT, HELPERS_MERCHANT_SCREEN);
        break;
    ...
    case (CLESS_SAMPLE_PAYWAVE_SCREEN_PIN_CANCEL):
		...
		ClessSample_Term_Read_Message(STD_MESS_PIN_CANCEL, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer(ERASE, HELPERS_CUSTOMER_LINE_1, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		Helper_RefreshScreen(NOWAIT, HELPERS_CUSTOMER_SCREEN);
        break;
    ...
    case (CLESS_SAMPLE_PAYWAVE_SCREEN_PIN_ERROR):
		if (auCustomerDisplayAvailable) // If a customer display is available
		{
//			ClessSample_Term_Read_Message(STD_MESS_PIN_ERROR, nMerchantLanguage, &tMsg);
//			Helper_DisplayTextMerchant(ERASE, HELPERS_MERCHANT_LINE_3, &tMsg, NOLEDSOFF);
//			Helper_RefreshScreen(NOWAIT, HELPERS_MERCHANT_SCREEN);
			MessageWithoutConfirmation("ERREUR PIN", NULL);
        }
        
//		ClessSample_Term_Read_Message(STD_MESS_PIN_ERROR, nCardholderLanguage, &tMsg);
//		Helper_DisplayTextCustomer(ERASE, HELPERS_CUSTOMER_LINE_1, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
//		Helper_RefreshScreen(NOWAIT, HELPERS_CUSTOMER_SCREEN);
    	if(FicPara.etat_PIN!=1) MessageWithoutConfirmation("ERREUR PIN", NULL);
        break;
...
*/
    ...
    case (CLESS_SAMPLE_VISAWAVE_SCREEN_PROCESSING):
		if (auCustomerDisplayAvailable) // If a customer display is available
		{
			ClessSample_Term_Read_Message(VISAWAVE_MESS_PROCESSING, nMerchantLanguage, &tMsg);
			Helper_DisplayTextMerchant(ERASE, HELPERS_MERCHANT_LINE_3, &tMsg, NOLEDSOFF);
			Helper_RefreshScreen(NOWAIT, HELPERS_MERCHANT_SCREEN);
		}
    ...
    case (CLESS_SAMPLE_VISAWAVE_SCREEN_COLLISION):        
		ClessSample_Term_Read_Message(VISAWAVE_MESS_SELECT_ONE_CARD_L1, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer(ERASE, HELPERS_CUSTOMER_LINE_2, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		ClessSample_Term_Read_Message(VISAWAVE_MESS_SELECT_ONE_CARD_L2, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer(NO_ERASE, HELPERS_CUSTOMER_LINE_3, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		Helper_RefreshScreen(NOWAIT, HELPERS_CUSTOMER_SCREEN);
        break;
        
    case (CLESS_SAMPLE_VISAWAVE_SCREEN_USE_CONTACT):
		ClessSample_Term_Read_Message(VISAWAVE_MESS_INSERT_OR_SWIPE, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer(ERASE, HELPERS_CUSTOMER_LINE_2, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		Helper_RefreshScreen(NOWAIT, HELPERS_CUSTOMER_SCREEN);
		//Os__xprinta("Here 5");
		Retour_Caisse("306");
    ...
    case (CLESS_SAMPLE_SCREEN_TRY_ANOTHER_CARD):
		...
		ClessSample_Term_Read_Message(STD_MESS_INSERT_OR_TRY_ANOTHER_CARD_L1, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer(ERASE, HELPERS_CUSTOMER_LINE_2, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		ClessSample_Term_Read_Message(STD_MESS_INSERT_OR_TRY_ANOTHER_CARD_L2, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer(NO_ERASE, HELPERS_CUSTOMER_LINE_3, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		Helper_RefreshScreen(NOWAIT, HELPERS_CUSTOMER_SCREEN);
        break;
...
//		Os__xprinta("Trace 6.2");
		ClessSample_Term_Read_Message(STD_MESS_ONLINE_DECLINED, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer (ERASE, HELPERS_CUSTOMER_LINE_2, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		Helper_RefreshScreen(NOWAIT, HELPERS_CUSTOMER_SCREEN);
		
		Os__xprinta("AUTORISATION 1");
		MessageWithoutConfirmation("AUTORISATION KO", NULL);
		break;
    ...
    case (CLESS_SAMPLE_EXPRESSPAY_SCREEN_NOT_PERMITTED):
    	if (auCustomerDisplayAvailable) // If a customer display is available
		{
			ClessSample_Term_Read_Message(EXPRESSPAY_MESS_CLESS_TRANS_NOT_PERMITTED_L1, nMerchantLanguage, &tMsg);
			Helper_DisplayTextMerchant(ERASE, HELPERS_MERCHANT_LINE_2, &tMsg, NOLEDSOFF);
			ClessSample_Term_Read_Message(EXPRESSPAY_MESS_CLESS_TRANS_NOT_PERMITTED_L2, nMerchantLanguage, &tMsg);
			Helper_DisplayTextMerchant(NO_ERASE, HELPERS_MERCHANT_LINE_3, &tMsg, NOLEDSOFF);
			Helper_RefreshScreen(NOWAIT, HELPERS_MERCHANT_SCREEN);
		}
		
		ClessSample_Term_Read_Message(EXPRESSPAY_MESS_CLESS_TRANS_NOT_PERMITTED_L1, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer (ERASE, HELPERS_CUSTOMER_LINE_2, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		ClessSample_Term_Read_Message(EXPRESSPAY_MESS_CLESS_TRANS_NOT_PERMITTED_L2, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer (NO_ERASE, HELPERS_CUSTOMER_LINE_3, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		Helper_RefreshScreen(NOWAIT, HELPERS_CUSTOMER_SCREEN);
		break;
	...
	case (CLESS_SAMPLE_EXPRESSPAY_SCREEN_FULL_ONLINE_PROCESSING):
    	if (auCustomerDisplayAvailable) // If a customer display is available
		{
			ClessSample_Term_Read_Message(EXPRESSPAY_MESS_PROCESSING, nMerchantLanguage, &tMsg);
			Helper_DisplayTextMerchant(ERASE, HELPERS_MERCHANT_LINE_3, &tMsg, NOLEDSOFF);
			ClessSample_Term_Read_Message(STD_MESS_PLEASE_WAIT, nMerchantLanguage, &tMsg);
			Helper_DisplayTextMerchant(NO_ERASE, HELPERS_MERCHANT_LINE_4, &tMsg, NOLEDSOFF);
			Helper_RefreshScreen(NOWAIT, HELPERS_MERCHANT_SCREEN);
		...
		ClessSample_Term_Read_Message(EXPRESSPAY_MESS_PROCESSING, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer (ERASE, HELPERS_CUSTOMER_LINE_2, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		ClessSample_Term_Read_Message(STD_MESS_PLEASE_WAIT, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer (NO_ERASE, HELPERS_CUSTOMER_LINE_3, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		Helper_RefreshScreen(NOWAIT, HELPERS_CUSTOMER_SCREEN);
...
//////////////////////////////
    ...
    case (CLESS_SAMPLE_INTERAC_SCREEN_COLLISION):
		ClessSample_Term_Read_Message(INTERAC_MESS_SELECT_ONE_CARD_L1, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer(ERASE, HELPERS_CUSTOMER_LINE_2, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		ClessSample_Term_Read_Message(INTERAC_MESS_SELECT_ONE_CARD_L2, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer(NO_ERASE, HELPERS_CUSTOMER_LINE_3, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		Helper_RefreshScreen(NOWAIT, HELPERS_CUSTOMER_SCREEN);
        break;

    case (CLESS_SAMPLE_INTERAC_SCREEN_NOT_SUPPORTED):
		ClessSample_Term_Read_Message(INTERAC_MESS_NOT_SUPPORTED_L1, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer(ERASE, HELPERS_CUSTOMER_LINE_2, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		ClessSample_Term_Read_Message(INTERAC_MESS_NOT_SUPPORTED_L2, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer(NO_ERASE, HELPERS_CUSTOMER_LINE_3, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		Helper_RefreshScreen(NOWAIT, HELPERS_CUSTOMER_SCREEN);
        break;

    case (CLESS_SAMPLE_INTERAC_SCREEN_CONTACT):
		ClessSample_Term_Read_Message(INTERAC_MESS_INSERT_CARD_L1, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer(ERASE, HELPERS_CUSTOMER_LINE_2, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		ClessSample_Term_Read_Message(INTERAC_MESS_INSERT_CARD_L2, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer(NO_ERASE, HELPERS_CUSTOMER_LINE_3, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		Helper_RefreshScreen(NOWAIT, HELPERS_CUSTOMER_SCREEN);
        break;
...
///////////////////////////
    ...
    case (CLESS_SAMPLE_VISA_SCREEN_SIGNATURE_ASIA):
    	if (auCustomerDisplayAvailable) // If a customer display is available
		{
			ClessSample_Term_Read_Message(VISAWAVE_MESS_SIGNATURE_CHECK_L1, nMerchantLanguage, &tMsg);
			Helper_DisplayTextMerchant(ERASE, HELPERS_MERCHANT_LINE_3, &tMsg, NOLEDSOFF);
			ClessSample_Term_Read_Message(VISAWAVE_MESS_SIGNATURE_CHECK_L2, nMerchantLanguage, &tMsg);
			Helper_DisplayTextMerchant(NO_ERASE, HELPERS_MERCHANT_LINE_4, &tMsg, NOLEDSOFF);
			Helper_RefreshScreen(NOWAIT, HELPERS_MERCHANT_SCREEN);
		}
		
		ClessSample_Term_Read_Message(VISAWAVE_MESS_SIGNATURE_CHECK_L1, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer(ERASE, HELPERS_CUSTOMER_LINE_2, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		ClessSample_Term_Read_Message(VISAWAVE_MESS_SIGNATURE_CHECK_L2, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer(NO_ERASE, HELPERS_CUSTOMER_LINE_3, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		Helper_RefreshScreen(NOWAIT, HELPERS_CUSTOMER_SCREEN);
        break;
...
//		Helper_RefreshScreen(NOWAIT, HELPERS_CUSTOMER_SCREEN);
	...
	case (CLESS_SAMPLE_VISA_APPROVED_ASIA):
		if (auCustomerDisplayAvailable) // If a customer display is available
		{
			ClessSample_Term_Read_Message(VISAWAVE_MESS_THANK_YOU, nMerchantLanguage, &tMsg);
			Helper_DisplayTextMerchant(ERASE, HELPERS_MERCHANT_LINE_1, &tMsg, NOLEDSOFF);
			ClessSample_Term_Read_Message(VISAWAVE_MESS_TRANSACTION, nMerchantLanguage, &tMsg);
			Helper_DisplayTextMerchant(NO_ERASE, HELPERS_MERCHANT_LINE_3, &tMsg, NOLEDSOFF);
			ClessSample_Term_Read_Message(VISAWAVE_MESS_TRANSACTION_COMPLETED, nMerchantLanguage, &tMsg);
			Helper_DisplayTextMerchant(NO_ERASE, HELPERS_MERCHANT_LINE_4, &tMsg, NOLEDSOFF);
			Helper_RefreshScreen(NOWAIT, HELPERS_MERCHANT_SCREEN);
		}
		ClessSample_Term_Read_Message(VISAWAVE_MESS_THANK_YOU, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer(ERASE, HELPERS_CUSTOMER_LINE_1, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		ClessSample_Term_Read_Message(VISAWAVE_MESS_TRANSACTION, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer(NO_ERASE, HELPERS_CUSTOMER_LINE_3, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		ClessSample_Term_Read_Message(VISAWAVE_MESS_TRANSACTION_COMPLETED, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer(NO_ERASE, HELPERS_CUSTOMER_LINE_4, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
	...
	case (CLESS_SAMPLE_VISA_ONLINE_APPROVED):
	{
		...
		if (auCustomerDisplayAvailable) // If a customer display is available
		{
			ClessSample_Term_Read_Message(nMsgNum, nMerchantLanguage, &tMsg);
			Helper_DisplayTextMerchant(ERASE, HELPERS_MERCHANT_LINE_3, &tMsg, NOLEDSOFF);
			Helper_RefreshScreen(NOWAIT, HELPERS_MERCHANT_SCREEN);
		}
		ClessSample_Term_Read_Message(nMsgNum, nCardholderLanguage, &tMsg);
//		Helper_DisplayTextCustomer(ERASE, HELPERS_CUSTOMER_LINE_1, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		MessageWithDelay(tMsg.message, 2);
...
//		if (TPass_GetDeviceType() == TPASS_CLESS_TYPE_IPP2XX) // Two lines display
	...
	case (CLESS_SAMPLE_VISA_SIGNATURE_KO_ASIA):
		if (auCustomerDisplayAvailable) // If a customer display is available
		{
			ClessSample_Term_Read_Message(VISAWAVE_MESS_SIGNATURE_REQUIRED, nMerchantLanguage, &tMsg);
			Helper_DisplayTextMerchant(ERASE, HELPERS_MERCHANT_LINE_2, &tMsg, NOLEDSOFF);
			ClessSample_Term_Read_Message(VISAWAVE_MESS_TRANSACTION, nMerchantLanguage, &tMsg);
			Helper_DisplayTextMerchant(NO_ERASE, HELPERS_MERCHANT_LINE_3, &tMsg, NOLEDSOFF);
			ClessSample_Term_Read_Message(VISAWAVE_MESS_TRANSACTION_NOT_COMPLETED, nMerchantLanguage, &tMsg);
			Helper_DisplayTextMerchant(NO_ERASE, HELPERS_MERCHANT_LINE_4, &tMsg, NOLEDSOFF);
			Helper_RefreshScreen(NOWAIT, HELPERS_MERCHANT_SCREEN);
		}
		ClessSample_Term_Read_Message(VISAWAVE_MESS_SIGNATURE_REQUIRED, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer(ERASE, HELPERS_CUSTOMER_LINE_1, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		ClessSample_Term_Read_Message(VISAWAVE_MESS_TRANSACTION, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer(NO_ERASE, HELPERS_CUSTOMER_LINE_2, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		ClessSample_Term_Read_Message(VISAWAVE_MESS_TRANSACTION_NOT_COMPLETED, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer(NO_ERASE, HELPERS_CUSTOMER_LINE_3, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		Helper_RefreshScreen(NOWAIT, HELPERS_CUSTOMER_SCREEN);
	break;
...
//		Helper_RefreshScreen(NOWAIT, HELPERS_CUSTOMER_SCREEN);
	...
	case (CLESS_SAMPLE_VISA_PIN_KO_ASIA):
		if (auCustomerDisplayAvailable) // If a customer display is available
		{
			ClessSample_Term_Read_Message(VISAWAVE_MESS_PIN_REQUIRED, nMerchantLanguage, &tMsg);
			Helper_DisplayTextMerchant(ERASE, HELPERS_MERCHANT_LINE_2, &tMsg, NOLEDSOFF);
			ClessSample_Term_Read_Message(VISAWAVE_MESS_TRANSACTION, nMerchantLanguage, &tMsg);
			Helper_DisplayTextMerchant(NO_ERASE, HELPERS_MERCHANT_LINE_3, &tMsg, NOLEDSOFF);
			ClessSample_Term_Read_Message(VISAWAVE_MESS_TRANSACTION_NOT_COMPLETED, nMerchantLanguage, &tMsg);
		...
		ClessSample_Term_Read_Message(VISAWAVE_MESS_PIN_REQUIRED, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer(ERASE, HELPERS_CUSTOMER_LINE_1, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		ClessSample_Term_Read_Message(VISAWAVE_MESS_TRANSACTION, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer(NO_ERASE, HELPERS_CUSTOMER_LINE_2, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		ClessSample_Term_Read_Message(VISAWAVE_MESS_TRANSACTION_NOT_COMPLETED, nCardholderLanguage, &tMsg);
	...
	case (CLESS_SAMPLE_VISA_DECLINED_ASIA):
		if (auCustomerDisplayAvailable) // If a customer display is available
		{
			ClessSample_Term_Read_Message(VISAWAVE_MESS_TRANSACTION, nMerchantLanguage, &tMsg);
			Helper_DisplayTextMerchant(ERASE, HELPERS_MERCHANT_LINE_1, &tMsg, NOLEDSOFF);
			ClessSample_Term_Read_Message(VISAWAVE_MESS_TRANSACTION_DECLINED, nMerchantLanguage, &tMsg);
			Helper_DisplayTextMerchant(NO_ERASE, HELPERS_MERCHANT_LINE_2, &tMsg, NOLEDSOFF);
			ClessSample_Term_Read_Message(VISAWAVE_MESS_USE_OTHER_VISA_CARD_L1, nMerchantLanguage, &tMsg);
			Helper_DisplayTextMerchant(NO_ERASE, HELPERS_MERCHANT_LINE_3, &tMsg, NOLEDSOFF);
			ClessSample_Term_Read_Message(VISAWAVE_MESS_USE_OTHER_VISA_CARD_L2, nMerchantLanguage, &tMsg);
			Helper_DisplayTextMerchant(NO_ERASE, HELPERS_MERCHANT_LINE_4, &tMsg, NOLEDSOFF);
			Helper_RefreshScreen(NOWAIT, HELPERS_MERCHANT_SCREEN);
		}
		ClessSample_Term_Read_Message(VISAWAVE_MESS_TRANSACTION, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer(ERASE, HELPERS_CUSTOMER_LINE_1, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		ClessSample_Term_Read_Message(VISAWAVE_MESS_TRANSACTION_DECLINED, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer(NO_ERASE, HELPERS_CUSTOMER_LINE_2, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		ClessSample_Term_Read_Message(VISAWAVE_MESS_USE_OTHER_VISA_CARD_L1, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer(NO_ERASE, HELPERS_CUSTOMER_LINE_3, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		ClessSample_Term_Read_Message(VISAWAVE_MESS_USE_OTHER_VISA_CARD_L2, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer(NO_ERASE, HELPERS_CUSTOMER_LINE_4, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		Helper_RefreshScreen(NOWAIT, HELPERS_CUSTOMER_SCREEN);
	break;
...
//		Os__xprinta("Trace 7.2");
//		ClessSample_Term_Read_Message(nMsgNum, nMerchantLanguage, &tMsg);
//		Helper_DisplayTextCustomer(ERASE, HELPERS_CUSTOMER_LINE_1, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		Os__xprinta("AUTORISATION 2");
		APEMV_UI_MessageDisplayDelay("AUTORISATION KO", 2);
...
//		if (TPass_GetDeviceType() == TPASS_CLESS_TYPE_IPP2XX) // Two lines display
	...
	case (CLESS_SAMPLE_VISA_ERROR_ASIA):
		if (auCustomerDisplayAvailable) // If a customer display is available
		{
			ClessSample_Term_Read_Message(VISAWAVE_MESS_TRANSACTION, nMerchantLanguage, &tMsg);
			Helper_DisplayTextMerchant(ERASE, HELPERS_MERCHANT_LINE_1, &tMsg, NOLEDSOFF);
			ClessSample_Term_Read_Message(VISAWAVE_MESS_TRANSACTION_TERMINATED, nMerchantLanguage, &tMsg);
			Helper_DisplayTextMerchant(NO_ERASE, HELPERS_MERCHANT_LINE_2, &tMsg, NOLEDSOFF);
			ClessSample_Term_Read_Message(VISAWAVE_MESS_USE_OTHER_VISA_CARD_L1, nMerchantLanguage, &tMsg);
			Helper_DisplayTextMerchant(NO_ERASE, HELPERS_MERCHANT_LINE_3, &tMsg, NOLEDSOFF);
			ClessSample_Term_Read_Message(VISAWAVE_MESS_USE_OTHER_VISA_CARD_L2, nMerchantLanguage, &tMsg);
		...
		ClessSample_Term_Read_Message(VISAWAVE_MESS_TRANSACTION, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer(ERASE, HELPERS_CUSTOMER_LINE_1, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		ClessSample_Term_Read_Message(VISAWAVE_MESS_TRANSACTION_TERMINATED, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer(NO_ERASE, HELPERS_CUSTOMER_LINE_2, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		ClessSample_Term_Read_Message(VISAWAVE_MESS_USE_OTHER_VISA_CARD_L1, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer(NO_ERASE, HELPERS_CUSTOMER_LINE_3, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		ClessSample_Term_Read_Message(VISAWAVE_MESS_USE_OTHER_VISA_CARD_L2, nCardholderLanguage, &tMsg);
	...
	if (nClessDeviceType != TPASS_CLESS_TYPE_NONE)
	{
		if (nClessDeviceType == TPASS_CLESS_TYPE_EMBEDDED)
		{
			...
			case (TERM_ISC250): // iSC250 ///////////////////////////////////////////////
				...
				*pInstructionLineCardholder = ((ClessSample_Goal_IsAvailable()) ? (HELPERS_CUSTOMER_LINE_5) : (HELPERS_CUSTOMER_LINE_4));
			...
			case (TERM_IWL280): // iWL280 ///////////////////////////////////////////////
				*pDisplayClessLogoInternal = TRUE;
				*pInstructionLineCardholder = ((ClessSample_Goal_IsAvailable()) ? (HELPERS_CUSTOMER_LINE_5) : (HELPERS_CUSTOMER_LINE_6));
				*pAmountLineCardholder = HELPERS_CUSTOMER_LINE_1;
				*pFontSizeCustomer = HELPERS_FONT_SIZE_MEDIUM;
				break;
			/////////////////////////////////////////////////////////////////////////////
			...
			case (TERM_EFT930S):
				*pDisplayClessLogoInternal = TRUE;
				*pInstructionLineCardholder = ((ClessSample_Goal_IsAvailable()) ? (HELPERS_CUSTOMER_LINE_5) : (HELPERS_CUSTOMER_LINE_2));
				*pAmountLineCardholder = HELPERS_CUSTOMER_LINE_1;
				*pFontSizeCustomer = HELPERS_FONT_SIZE_MEDIUM;
				break;
			/////////////////////////////////////////////////////////////////////////////
			case (TERM_SPM): // iPA280 //////////////////////////////////////////////////
				*pDisplayClessLogoInternal = FALSE;
				*pInstructionLineCardholder = ((ClessSample_Goal_IsAvailable()) ? (HELPERS_CUSTOMER_LINE_5) : (HELPERS_CUSTOMER_LINE_3));
