...
int CS_Settings_getListOfFiles (const char * archiveName, T_NAMES_LIST * list)
{
	...
	sprintf (archiveFileFullName, "%s%s%s%s", m_pFileType, m_pHostPath, archiveName, m_pTarExtension);
...
int CS_Settings_loadFromFile (const char * archiveName, const char * fileName, TLV_TREE_NODE * loadedSettingTree, T_SETTINGS_DECODER decoderType, const char * fileExtension)
{
	...
	sprintf (archiveFullName, "%s%s%s%s", m_pFileType, m_pHostPath, archiveName, m_pTarExtension);
    ...
    sprintf (tarFullName, "%s%s%s%s/%s", m_pFileType, m_pHostPath, archiveName, m_pTarExtension, fileName);
    ...
    sprintf (fullFileExtension, "%s%s", fileExtension, m_pGzExtension);
    	...
    	(fileExtensionLength == 0))
    {
        ...
        // Remove extension
        sprintf (fileNameNoGz, "%s", fileName);
        fileNameNoGz[strlen(fileNameNoGz)-3] = '\0';

        sprintf (tarFullNameGz, "%s/%s", m_pTempGzFileFullName, fileNameNoGz);
    ...
    switch (decoderType)
    {
    	...
    	case (SettingsDecoderDekDet):
			ClessSample_DataExchange_DecodeXMLBuffer(loadedSettingTree, pBuffer, fileLength);
			break;
...
int CS_Settings_SaveSelectedFileName (const char * archiveName, const char * fileName)
{
	...
	sprintf (savedArchiveFileName, "%s%s%s%s", m_pFileType, m_pHostPath, archiveName, m_pSavExtension);
	sprintf (savedArchiveFileNameTemp, "%s%s%s%s", m_pFileType, m_pHostPath, archiveName, m_pTempExtension);
