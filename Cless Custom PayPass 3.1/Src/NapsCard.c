...
//		Os__xprintaa("recup");
//		Os__xprintaa(StructInfoPayment.Normal_Iso2);

	};
//	Os__xprintaa("p_puc_buffer");
//	Os__xprintaa(p_puc_buffer);
	p_puc_buffer[l_s_Lg_iso1+l_c_i]='=';
...
UC   Recup_Info_badge(PUC p_puc_buffer,UC init)
{
	...
	if(init)
	{
		if(tst_fic_vide()==OK)
		{
			...
			{
				//if(g_trace)Os__xprint("LG. NUM. COMM. INCORRECT");
				return KO;
			}
			...
			{
				//if(g_trace)Os__xprint("LG. PT. VENTE INCORRECT");
				return KO;
			}
			...
			{
				//if(g_trace)Os__xprint("LG. NUM. TERM. INCORRECT");
				return KO;
			}
		}
	}
...
uchar	PrefixControl(uchar*	Card)
{
	...
	//if(StructInfoPayment.Code_Prefixe == 44)
		return OK ;
	//else
		//MessageWithDelay("Inserer la carte", NULL, 6*1000);
		//return KO ;
...
uchar	BinPrefixControl(uchar *Card,C message)
{
	...
	if(BinControl(Card)!=OK)
	{
		if(
			...
			(StructInfoPayment.Entry_Mode != TRS_SCT)
...
uchar  Saisie_Info_Carte(char codefunction)
{
	...
	do
	{
   		ptKeyboard = Os__xget_varkey(103,19);
		if(ptKeyboard[0]==KEY_CLEAR)
			return KO;
	}while((strlen((PC)StructInfoPayment.Enter)-1)<13);
	StructInfoPayment.Holder_Card[0] = strlen((PC)StructInfoPayment.Enter)-1;
	memcpy(&StructInfoPayment.Holder_Card[1],StructInfoPayment.Enter+1,StructInfoPayment.Holder_Card[0]);


//	//numero de la carte suite
//	CUMORE_CMI_CARD_NUMBER((char*)&StructInfoPayment.Holder_Card[1],1);
//	ptKeyboard = Os__xget_varkey(103,3);
//	if(ptKeyboard[0]==KEY_CLEAR)
//	{
//		CUMORE_CMI_CARD_NUMBER((char *)&StructInfoPayment.Holder_Card[1],2);
//		return KO;
//	}
//	StructInfoPayment.Holder_Card[0] = StructInfoPayment.Holder_Card[0]+ strlen((PC)StructInfoPayment.Enter)-1;
//	strcat((char*)&StructInfoPayment.Holder_Card[1],(char*)(StructInfoPayment.Enter+1));
//
//	CUMORE_CMI_CARD_NUMBER((char*)&StructInfoPayment.Holder_Card[1],2);
	// Traitement clé de luhn
	if(luhn_key_ok(StructInfoPayment.Holder_Card)!=TRUE)
	...
	do
	{
		ptKeyboard = Os__xget_varkey(104,4);
		if(ptKeyboard[0]==KEY_CLEAR)
			return KO;
	}while((strlen((PC)StructInfoPayment.Enter)-1)<4);
	memcpy(StructInfoPayment.Expired_Date,StructInfoPayment.Enter+1+2,2);
	memcpy(StructInfoPayment.Expired_Date+2,StructInfoPayment.Enter+1,2);
	if(
...
//	APEMV_UI_MessageDisplayDelay(pin,2);
//	APEMV_UI_MessageDisplayDelay(StructInfoPayment.Holder_Card+1,2);
