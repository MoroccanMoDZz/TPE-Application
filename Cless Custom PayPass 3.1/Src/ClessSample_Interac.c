...
static int __ClessSample_Interac_RetreiveCvmToApply (T_SHARED_DATA_STRUCT * pResultDataStruct, unsigned char * pCvm);
...
static T_Bool __ClessSample_Interac_OnlinePinManagement (T_SHARED_DATA_STRUCT * pStructureForOnlineData, int nCardHolderLang)
{
	...
	if (pDataRequest != NULL)
	{
		...
		{
			GTL_Traces_TraceDebug("__ClessSample_Interac_OnlinePinManagement : An error occured when getting tags from the Interac kernel (cr=%02x)", cr);
			nResult = B_FALSE;
			goto End;
		}
...
void ClessSample_Interac_DebugActivation (int bActivate)
{
	...
	if (pSharedStructure != NULL)
	{
		...
		else
		{
			...
			{
				GTL_Traces_TraceDebug("ClessSample_Interac_DebugActivation : Error occured during Interac Debug activation (nResult = %02x)", nResult);
			}
		}
...
static void __ClessSample_Interac_GetSelectedPreferedLanguage (int * pCardholderLanguage, int nMerchantLanguage)
{
	...
	*pCardholderLanguage = nMerchantLanguage;
	...
	if (pStruct == NULL)
	{
		...
		GTL_Traces_TraceDebug ("__ClessSample_Interac_GetSelectedPreferedLanguage : An error occured when creating the shared buffer");
		goto End;
	}	
	...
	if (ret != STATUS_SHARED_EXCHANGE_OK)
	{
		...
		GTL_Traces_TraceDebug ("__ClessSample_Interac_GetSelectedPreferedLanguage : An error occured when adding the TAG_KERNEL_SELECTED_PREFERED_LANGUAGE tag in the structure (ret = %02x)", ret);
		goto End;
	}	
	...
	{
		// An error occured when getting data from the Interac kernel
		GTL_Traces_TraceDebug ("__ClessSample_Interac_GetSelectedPreferedLanguage : An error occured when getting data from the Interac kernel (ret = %02x)", ret);
		goto End;
	}
...
int ClessSample_Interac_PerformTransaction (T_SHARED_DATA_STRUCT * pDataStruct)
{
	...
	{		
		...
		if (!__ClessSample_Interac_RetreiveCvmToApply (pDataStruct, &ucCvm))
		{
...
//						Helper_RefreshScreen(WAIT, HELPERS_CUSTOMER_SCREEN);
				...
				else
				{
					if (auCustomerDisplayAvailable) // If a customer display is available
					{
						ClessSample_Term_Read_Message(STD_MESS_UNABLE_ON_LINE, merchLang, &tMsg);
						Helper_DisplayTextMerchant(ERASE, HELPERS_MERCHANT_LINE_3, &tMsg, LEDSOFF);
						Helper_RefreshScreen(NOWAIT, HELPERS_MERCHANT_SCREEN);
					}
//					ClessSample_Term_Read_Message(STD_MESS_ONLINE_DECLINED, nCardHolderLang, &tMsg);
...
//					Helper_RefreshScreen(WAIT, HELPERS_CUSTOMER_SCREEN);
		...
		case (KERNEL_STATUS_CARD_BLOCKED):
			if (auCustomerDisplayAvailable) // If a customer display is available
			{
				ClessSample_Term_Read_Message(STD_MESS_CARD_BLOCKED, merchLang, &tMsg);
				Helper_DisplayTextMerchant(ERASE, HELPERS_MERCHANT_LINE_3, &tMsg, LEDSOFF);
				Helper_RefreshScreen(NOWAIT, HELPERS_MERCHANT_SCREEN);
			}
			ClessSample_Term_Read_Message(STD_MESS_CARD_BLOCKED, nCardHolderLang, &tMsg);
			Helper_DisplayTextCustomer (ERASE, HELPERS_CUSTOMER_LINE_2, &tMsg, CLESSSAMPLE_ALIGN_CENTER, LEDSOFF);
		...
		case (KERNEL_STATUS_APPLICATION_BLOCKED):
			if (auCustomerDisplayAvailable) // If a customer display is available
			{
				ClessSample_Term_Read_Message(STD_MESS_APPLICATION_BLOCKED, merchLang, &tMsg);
				Helper_DisplayTextMerchant(ERASE, HELPERS_MERCHANT_LINE_3, &tMsg, LEDSOFF);
				Helper_RefreshScreen(NOWAIT, HELPERS_MERCHANT_SCREEN);
			}
			ClessSample_Term_Read_Message(STD_MESS_APPLICATION_BLOCKED, nCardHolderLang, &tMsg);
			Helper_DisplayTextCustomer (ERASE, HELPERS_CUSTOMER_LINE_2, &tMsg, CLESSSAMPLE_ALIGN_CENTER, LEDSOFF);
			Helper_RefreshScreen(NOWAIT, HELPERS_CUSTOMER_SCREEN);
			HelperErrorSequence (WITHBEEP);
			Helper_WaitUserDisplay();
