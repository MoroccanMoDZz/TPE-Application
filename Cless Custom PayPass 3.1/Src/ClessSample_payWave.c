...
static int __ClessSample_payWave_RetreiveSignature (T_SHARED_DATA_STRUCT * pResultDataStruct, unsigned char * pSignature);
...
extern DEL pt_AuthoDEL;	//KamalCless
...
static int __ClessSample_payWave_RetreiveSignature (T_SHARED_DATA_STRUCT * pResultDataStruct, unsigned char * pSignature)
{
	...
	* pSignature = 0; // Default result : no signature
	...
	if (cr != STATUS_SHARED_EXCHANGE_OK)
	{
		GTL_Traces_TraceDebug("__ClessSample_payWave_RetreiveSignature : Unable to get SigantureRequested from the payWave kernel response (cr = %02x)", cr);
		nResult = FALSE;
		goto End;
	}
...
static int __ClessSample_payWave_PinOnLineRequiredState (T_SHARED_DATA_STRUCT * pResultDataStruct, unsigned char * pPinOnLineRequiredState)
{
	...
	* pPinOnLineRequiredState = 0; // Default result : no PinOnLineRequired
	...
	if (cr != STATUS_SHARED_EXCHANGE_OK)
	{
		GTL_Traces_TraceDebug("__ClessSample_payWave_RetreiveSignature : Unable to get OnLinePinRequested from the payWave kernel response (cr = %02x)", cr);
		nResult = FALSE;
		goto End;
	}
...
void ClessSample_payWave_DebugActivation (int bActivate)
{
	...
	if (pSharedStructure != NULL)
	{
		...
		else
		{
			...
			{
				GTL_Traces_TraceDebug("ClessSample_payWave_DebugActivation : Error occured during payWave Debug activation (nResult = %02x)", nResult);
			}
		}
...
int ClessSample_payWave_CustomiseStep (T_SHARED_DATA_STRUCT * pSharedData, const unsigned char ucCustomisationStep)
{
    ...
    case STEP_PAYWAVE_QVSDC_REMOVE_CARD:
		HelperRemoveCardSequenceTwoLines(pSharedData);
	...
	case (STEP_PAYWAVE_QVSDC_BLACK_LIST_CONTROL):
		...
		if (ClessSample_BlackListIsPan((int)ulLgPan, pPan, (int)(*pPanSeqNumber)) == TRUE)
		{
			...
			if (cr != STATUS_SHARED_EXCHANGE_OK)
			{
				GTL_Traces_TraceDebug("ClessSample_payWave_CustomiseStep An error occured when adding TAG_KERNEL_PAN_IN_BLACK_LIST in the shared buffer (cr = %02x)", cr);
				GTL_SharedExchange_ClearEx (pSharedData, FALSE);
				break;
			}
			nCardInBlackList = 1;
...
static int __ClessSample_payWave_WaitClessCard(T_SHARED_DATA_STRUCT * pDataStruct)
{
	...
	do{ // Infinite loop to wait the appropriate card (with good AID ....)
		...
		{
			HelperRemoveCardSequenceTwoLines(pDataStruct);
...
EndCardTreatment:
	...
	return(nAppropriateCardFounded);
}
...
int ClessSample_PayWave_PerformTransaction (T_SHARED_DATA_STRUCT * pDataStruct)
{
	...
	else
	{
		...
		// 
		//ClessSample_Scan_TransacOpenDrivers();//KamalCless
		...
		ucSignature = 0;
		__ClessSample_payWave_RetreiveSignature (pDataStruct, &ucSignature);
		...
		{			
			...
			case USER_EVENT:	// User event (from main task), it indicates the task is going to be killed (because g_bScanning_task_to_be_killed has been set to TRUE by the custom application
				/* ClessSample_Term_Read_Message(STD_MESS_USER, merchLang, &tDisplayMsg);
			...
			case KEYBOARD:		// Keyboard event
				/* ClessSample_Term_Read_Message(STD_MESS_KEYBOARD, merchLang, &tDisplayMsg);
				Helper_DisplayTextMerchant(ERASE, HELPERS_MERCHANT_LINE_3, &tDisplayMsg, NOLEDSOFF);
				Helper_RefreshScreen(WAIT, HELPERS_MERCHANT_SCREEN);*/
			...
			default:
				/* ClessSample_Term_Read_Message(STD_MESS_STOPPED, merchLang, &tDisplayMsg);
				Helper_DisplayTextMerchant(ERASE, HELPERS_MERCHANT_LINE_3, &tDisplayMsg, NOLEDSOFF);
		...
		else 
		{
			...
			case KERNEL_STATUS_OK:
				...
				{
					ClessSample_Term_Read_Message(STD_MESS_ERROR_STATUS, merchLang, &tDisplayMsg);
					Helper_DisplayTextMerchant(ERASE, HELPERS_MERCHANT_LINE_3, &tDisplayMsg, LEDSOFF);
					Helper_RefreshScreen(NOWAIT, HELPERS_MERCHANT_SCREEN);
				}
				ClessSample_Term_Read_Message(STD_MESS_ERROR_STATUS, nCardHolderLang, &tDisplayMsg);
...
//Integration VISA
				payWave_GetAllData(pDataStruct);
				ClessSample_Receipt_PrintPayPass (pDataStruct, 0, 0, 1);
				
//               ClessSample_GuiState_DisplayScreen (CLESS_SAMPLE_SCREEN_ERROR_STATUS, merchLang, nCardHolderLang);
//
...
//				}
			...
			case (KERNEL_STATUS_ONLINE_AUTHORISATION):
				...
				{
					APEMV_UI_MessageDisplayDelay ("OnLine Pin Requested", 1);
...
/*
					...
					if (!bOnlinePinError)
					{
						...
						//	Helper_DisplayTextCustomer(ERASE, HELPERS_CUSTOMER_LINE_1, &tDisplayMsg, CLESSSAMPLE_ALIGN_CENTER, LEDSOFF);
						//	Helper_RefreshScreen(NOWAIT, HELPERS_CUSTOMER_SCREEN);o/
						//
...
*/
				...
				{
/*					APEMV_UI_MessageDisplayDelay ("OnLine Pin Forced", 1);
...
//					Helper_Visa_OnlineProcessingIHM(HELPERS_PAYWAVE, pDataStruct, auCustomerDisplayAvailable,merchLang,nCardHolderLang);
					...
					Remplir_Infopayment_Cless_VISA(pDataStruct,nCardHolderLang);
					iRet = valide_carte();//badr
					if(iRet == TRANS_FILE_FULL)/*copie famri *///Version 07.28s
					    {
...
//					{
//						l_retour = KO;
//						APEMV_UI_MessageDisplayDelay("USE CONTACT \n INTERFACE",2);
//					}
//					else
					{
...
/* ???? */              if (cr_pin_online == CANCEL_INPUT)
                            ClessSample_GuiState_DisplayScreen (CLESS_SAMPLE_PAYWAVE_SCREEN_PIN_CANCEL, merchLang, nCardHolderLang);
                        else
                            ClessSample_GuiState_DisplayScreen (CLESS_SAMPLE_PAYWAVE_SCREEN_PIN_ERROR, merchLang, nCardHolderLang);
					}
					else
					{
					...
					GTL_Traces_TraceDebug ("ClessSample_PayWave_PerformTransaction : An error occured for PIN Entry");
				}
			...
			case (KERNEL_STATUS_MOBILE):
				if (ClessSample_Common_RetrieveInfo (pDataStruct, TAG_PAYWAVE_TERMINAL_TRANSACTION_QUALIFIERS, &pttq))
				{
					if (pttq[0] & 0x20) // If qVSDC supported
					{
						/* if (auCustomerDisplayAvailable) // If a customer display is available
						{
							ClessSample_Term_Read_Message(VISAWAVE_MESS_PHONE_INSTRUCTIONS_L1, merchLang, &tDisplayMsg);
							Helper_DisplayTextMerchant(ERASE, HELPERS_MERCHANT_LINE_2, &tDisplayMsg, LEDSOFF);
							ClessSample_Term_Read_Message(VISAWAVE_MESS_PHONE_INSTRUCTIONS_L2, merchLang, &tDisplayMsg);
							Helper_DisplayTextMerchant(NO_ERASE, HELPERS_MERCHANT_LINE_3, &tDisplayMsg, LEDSOFF);
							Helper_RefreshScreen(NOWAIT, HELPERS_MERCHANT_SCREEN);
						}
						ClessSample_Term_Read_Message(VISAWAVE_MESS_PHONE_INSTRUCTIONS_L1, nCardHolderLang, &tDisplayMsg);
						Helper_DisplayTextCustomer(ERASE, HELPERS_CUSTOMER_LINE_1, &tDisplayMsg, CLESSSAMPLE_ALIGN_CENTER, LEDSOFF);
						ClessSample_Term_Read_Message(VISAWAVE_MESS_PHONE_INSTRUCTIONS_L2, nCardHolderLang, &tDisplayMsg);
						Helper_DisplayTextCustomer(NO_ERASE, HELPERS_CUSTOMER_LINE_2, &tDisplayMsg, CLESSSAMPLE_ALIGN_CENTER, LEDSOFF);
						Helper_RefreshScreen(NOWAIT, HELPERS_CUSTOMER_SCREEN);*/
						
                        ClessSample_GuiState_DisplayScreen (CLESS_SAMPLE_SCREEN_PHONE_INSTRUCTIONS, merchLang, nCardHolderLang);
...
//!	\return
//!		- payWave kernel result.
void ClessSample_payWave_AidRelatedData(T_SHARED_DATA_STRUCT * pSharedStruct, T_SHARED_DATA_STRUCT * pAidParameters)
{
	// Local variables
	int cr;
...
/*
					switch(l_retour)
					{
						...
						memcpy(StructInfoPayment.Approval_Code,R_Transac8583.champs38,6);
						{
							...
							if (FindTag (TAG_EMV_AUTHORISATION_RESPONSE_CODE, &Position, &Len,R_Transac8583.champs55, R_Transac8583.l_champs55) == OK)
							{
					...
					case REFERRAL :
						if(g_trace)Os__xprint("Referral");
				...
				//		Status = OK;// stouargui le 160418
						Status = KO;
						memcpy(StructInfoPayment.AuthRespCode8A,"01",2);
						if(
							(StructInfoPayment.CodeFunction == PREAUTHCODE)
							||(StructInfoPayment.CodeFunction == PREAUTHCOMPCODE)
							)
				...
				//		Status = OK; // stouargui le 160418
					...
					case FORMAT_MSG_REC_KO:
						...
						StructInfoPayment.Flag_Dem_Aut = 0;
						memcpy(StructInfoPayment.AuthRespCode8A,"51",2);
						memcpy(R_Transac8583.champs39,"909",3); //stouargui add le 010418 RESEAU NOK >>>>>909
						Status = KO;
						break;
