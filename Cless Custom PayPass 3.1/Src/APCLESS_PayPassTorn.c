...
static int __APCLESS_PayPassTorn_CleanOldRecord(Telium_Date_t *currentDate, int lifeTime);
...
static int __APCLESS_PayPassTorn_CleanOldRecord (Telium_Date_t *currentDate, int lifeTime)
{
   ...
   if(__APCLESS_PayPassTorn_transactionLog != NULL)
   {
      ...
      if (tlvRecord != NULL)
      {
         ...
         // Copy the current transaction Date/Time in a Telium_Date_t structure
