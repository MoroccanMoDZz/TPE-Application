...
{
   int result = FALSE;
   unsigned char termCapabilities[] = {0xE0, 0x28 , 0xE8}; //68,C8
   unsigned char termcountrycode[] = {0x05, 0x04};
   ...
   {  // Add PayPass tags
      ...
      unsigned char aidPaypassFloorLimit[]= {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};			//0,00
      unsigned char aidPaypassTrnLimitNoDcv[] = {0x99, 0x99, 0x99, 0x99, 0x99, 0x99};		//9999999999,99
      unsigned char aidPaypassTrnLimitDcv[] = {0x99, 0x99, 0x99, 0x99, 0x99, 0x99};			//9999999999,99
      unsigned char aidPaypassCvmRequiredLimit[] = {0x00, 0x00, 0x00, 0x06, 0x00, 0x00};	//200,00		//Version 07.28
      unsigned char aidPaypassSecurityCapability[] = {0xF8};
      unsigned char aidPaypassEmvIntTacDefault[] = {0xFC,0x50,0x80,0x88,0x00};
      unsigned char aidPaypassEmvIntTacDenial[] = {0x00, 0x00, 0x00, 0x00, 0x00};
      unsigned char aidPaypassEmvIntTacOnline[] = {0xFC,0x50,0x80,0x88,0x00}; //{0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
      unsigned char aidPaypassCardDataInputCapability[] = {0xE0};
      unsigned char aidPaypassMChipCvmCapabilityCvmRequired[] = {0x60}; //0x68
      unsigned char aidPaypassMChipCvmCapabilityCvmNotRequired[] = {0xE8};//{0xE8};
      unsigned char aidPaypassKernelConfiguration[] = {0x00};//{0x00};//omoubarak 241121
      unsigned char aidPaypassCAPKIndexList[] = {0xEF,0xFA};
      unsigned char aidPaypassRiskManagement[]={0x6C,0xF8,0x80,0x00,0x00,0x00,0x00,0x00};
	  unsigned char StepCustom[KERNEL_PAYMENT_FLOW_CUSTOM_LENGTH];//omoubarak 241121
      ...
      if(GTL_SharedExchange_AddTag(buffer, TAG_EMV_TERMINAL_COUNTRY_CODE, sizeof(termcountrycode), termcountrycode) != STATUS_SHARED_EXCHANGE_OK) goto End;
      if(GTL_SharedExchange_AddTag(buffer, TAG_EMV_TERMINAL_RISK_MANAGEMENT_DATA, sizeof(aidPaypassRiskManagement), aidPaypassRiskManagement) != STATUS_SHARED_EXCHANGE_OK) goto End;
      //omoubarak 241121
      ADD_STEP_CUSTOM(STEP_PAYPASS_MCHIP_SEND_PHONE_MSG, StepCustom);		// To do GUI when MChip phone has been read
      ADD_STEP_CUSTOM(STEP_PAYPASS_MSTRIPE_SEND_PHONE_MSG, StepCustom);
      result = TRUE;
   ...
   {  // Add payWave tags
      ...
      unsigned char aid_EP_TERMINAL_TRANSACTION_QUALIFIERS[]		 	= {0xB6, 0x80, 0xC0, 0x00}; 							// <!-- TAG_EP_TERMINAL_TRANSACTION_QUALIFIERS clss MSD + qVSDC & onlinePIN + sign supported + CVN17 + ISP-->"
      unsigned char aid_EP_CLESS_TRANSACTION_LIMIT[] 					= {0x00, 0x00, 0x00, 0x02, 0x00, 0x00}; 				// <!-- TAG_EP_CLESS_TRANSACTION_LIMIT : 100 euros -->"
      unsigned char aid_EP_CLESS_FLOOR_LIMIT_2[] 						= {0x00, 0x00, 0x00, 0x01, 0x50, 0x00}; 				// <!-- TAG_EP_CLESS_FLOOR_LIMIT : 15 euros -->"
      unsigned char aid_EP_CLESS_CVM_REQUIRED_LIMIT_2[]					= {0x00, 0x00, 0x00, 0x06, 0x00, 0x00}; //Version 07.28	// <!-- TAG_EP_CLESS_CVM_REQUIRED_LIMIT: 10 euros -->"
      unsigned char aid_EMV_TERMINAL_FLOOR_LIMIT_2[]					= {0x00, 0x00, 0x05, 0xAA}; 							// <!-- TAG_EMV_TERMINAL_FLOOR_LIMIT : 14,50 euros -->"
      ...
      // TAG_PAYPASS_CLESS_TRANSACTION_LIMIT_NO_DCV : ?????????
      if(GTL_SharedExchange_AddTag(buffer, TAG_EP_TERMINAL_TRANSACTION_QUALIFIERS, sizeof(aid_EP_TERMINAL_TRANSACTION_QUALIFIERS), aid_EP_TERMINAL_TRANSACTION_QUALIFIERS) != STATUS_SHARED_EXCHANGE_OK) goto End;
      // TAG_PAYPASS_CLESS_TRANSACTION_LIMIT_DCV : ?????????
      if(GTL_SharedExchange_AddTag(buffer, TAG_EP_CLESS_TRANSACTION_LIMIT, sizeof(aid_EP_CLESS_TRANSACTION_LIMIT), aid_EP_CLESS_TRANSACTION_LIMIT) != STATUS_SHARED_EXCHANGE_OK) goto End;
      // TAG_EP_CLESS_CVM_REQUIRED_LIMIT : ?????????
      if(GTL_SharedExchange_AddTag(buffer, TAG_EP_CLESS_FLOOR_LIMIT, sizeof(aid_EP_CLESS_FLOOR_LIMIT_2), aid_EP_CLESS_FLOOR_LIMIT_2) != STATUS_SHARED_EXCHANGE_OK) goto End;
      // TAG_EP_CLESS_FLOOR_LIMIT : ?????????
      if(GTL_SharedExchange_AddTag(buffer, TAG_EP_CLESS_CVM_REQUIRED_LIMIT, sizeof(aid_EP_CLESS_CVM_REQUIRED_LIMIT_2), aid_EP_CLESS_CVM_REQUIRED_LIMIT_2) != STATUS_SHARED_EXCHANGE_OK) goto End;
      // TAG_EMV_TERMINAL_FLOOR_LIMIT : ?????????
      if(GTL_SharedExchange_AddTag(buffer, TAG_EMV_TERMINAL_FLOOR_LIMIT, sizeof(aid_EMV_TERMINAL_FLOOR_LIMIT_2), aid_EMV_TERMINAL_FLOOR_LIMIT_2) != STATUS_SHARED_EXCHANGE_OK) goto End;
