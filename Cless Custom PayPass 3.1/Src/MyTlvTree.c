...
//===========================================================================
#ifdef _TLVTREE_DEBUG_
void Dbg_TlvTree_ReleaseEx(TLV_TREE_NODE* pTlvTree, const char* szFile, int nLine)
#else
...
#endif
{
	if (pTlvTree != NULL)
	{
		...
		{
#ifdef _TLVTREE_DEBUG_
			Dbg_TlvTree_Release(*pTlvTree, szFile, nLine);
#else
...
void TlvTree_DumpAllocatedTrees(void)
{
	...
	while(hNode != NULL)
	{
		...
		*pPtr = '0';
		...
		if (pTlvTree != NULL)
		{
			pPtr += sprintf(pPtr, "%08lX ", (unsigned long)*pTlvTree);
		}
		if (pLine != NULL)
		{
			pPtr += sprintf(pPtr, "%d ", *pLine);
		}
		if (pFile != NULL)
		{
			pPtr += sprintf(pPtr, "%s", pFile);
		}
...
static void __Dbg_TlvTree_Release(TLV_TREE_NODE hTlvTree, const char* szFile, int nLine)
{
	...
	if (bUnknown)
	{
		trace(SAP_KERNEL_TRAP_RESET, 15, "UNKNOWN TlvTree to release");

		pPtr = String;
		*pPtr = '0';
		pPtr += sprintf(pPtr, "%08lX ", (unsigned long)hTlvTree);
		if (nLine > 0)
			pPtr += sprintf(pPtr, "%d ", nLine);
		if (szFile != NULL)
			pPtr += sprintf(pPtr, "%s", szFile);
...
//===========================================================================
int Dbg_TlvTree_Graft(TLV_TREE_NODE hParent, TLV_TREE_NODE hChild, const char* szFile, int nLine)
{
	int nResult;

	nResult = TlvTree_Graft(hParent, hChild);
	if (nResult == TLV_TREE_OK)
	{
		__Dbg_TlvTree_Release(hChild, szFile, nLine);
	}
...
void Dbg_TlvTree_Release(TLV_TREE_NODE hNode, const char* szFile, int nLine)
{
	if (hNode != NULL)
	{
		if (TlvTree_GetParent(hNode) == NULL)
			__Dbg_TlvTree_Release(hNode, szFile, nLine);
	}
	else __Dbg_TlvTree_Release(hNode, szFile, nLine);
...
void TraceTLVTree(int i_x_service_number, TLV_TREE_NODE *ptr_Tree)
{
	...
	// printer trace 
	Telium_Pprintf(Buffer);
	// trace(0, sizeof(Buffer), Buffer);
...
void TraceFreeRam(int pos)
{
	...
	free_ram = FreeSpace();
	sprintf(buf, "Pos:%d Free ram:%ldKo\n", pos, (free_ram/1024));
