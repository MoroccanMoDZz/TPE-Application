...
int cu_My_PSQ_Give_Language(void)
{
       const char *lang;
...
//	APEMV_UI_MessageDisplayDelay("Topen.02",2);

//	APEMV_UI_MessageDisplayDelay("Topen.02.5",2);
	if(lp_keybd != NULL)
	{
		//APEMV_UI_MessageDisplayDelay("Topen.03",2);
		// Test if a key is already pressed
		Telium_Status(lp_keybd, &state);

		//APEMV_UI_MessageDisplayDelay("Topen.04",2);
		if (state == KEYS_IN_BUFFER)
		// No wait needed, get immediatelly the key pressed
		{
			//APEMV_UI_MessageDisplayDelay("Topen.05",2);
		...
		{
			//APEMV_UI_MessageDisplayDelay("Topen.06",2);
			if (timeout != 0)
			{
				...
				case KEYBOARD :
					...
					default:
					//trace
					//	Os__xprinta("timmmmmmmmmmmmmout");
					res_key = B_FALSE;
					FicPara.etat_PIN=1;
					//*xp_key= T_ANN;
					break;
	...
	//APEMV_UI_MessageDisplayDelay("Topen.07",2);
