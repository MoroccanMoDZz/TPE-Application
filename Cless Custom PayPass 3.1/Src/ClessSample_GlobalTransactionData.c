...
void Cless_PSQ_Est_money (S_MONEY *p_money)
{
	...
	memcpy(p_money->nom,"MAD",3);
	memcpy(p_money->code,"504",3);
	p_money->posdec = 2;
	...
	{
		memcpy(p_money->nom,"EUR",3);
		memcpy(p_money->code,"978",3);
		p_money->posdec = 2;
	}
	else if(!memcmp(StructInfoCaisse[0].DE,"840",3))
	{
		memcpy(p_money->nom,"USD",3);
		memcpy(p_money->code,"840",3);
		p_money->posdec = 2;
	}
	else
	{
		memcpy(p_money->nom,"ERR",3);
		memcpy(p_money->code,"000",3);
		p_money->posdec = 2;
	}
}
	...
	if (CS_GlobalTransactionData_Init())
	{
		...
		else
		{
			...
			else
			{
				...
				if (ClessSample_Unattended_IsUnattendedMechanism())
				{
					// If unattended, the transaction amount authorized is already available, and no entry is necessary
			...
			if (!ClessSample_Batch_AddTscToSharedBuffer(gs_pGlobalTransactionData))
			{
				GTL_Traces_TraceDebug("CS_GlobalTransactionData_Prepare : ClessSample_Batch_AddTscToSharedBuffer failed");
				nResult = FALSE;
				goto end;
			}
		}
