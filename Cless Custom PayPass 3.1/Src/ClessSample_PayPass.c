...
static void __ClessSample_PayPass_DumpOps (unsigned char * pOpsValue, unsigned long ulOpsLength)
{
	...
	{
		ClessSample_DumpData ("OUTCOME PARAMETER SET (OUT)");
...
static void __ClessSample_PayPass_DumpUird (unsigned char * pUirdValue, unsigned long ulUirdLength, const int bIsOutUird)
{
	(void) ulUirdLength;
	...
	{
		if (bIsOutUird)
			ClessSample_DumpData ("USER INTERFACE REQ DATA (OUT)");
		else
			ClessSample_DumpData ("USER INTERFACE REQ DATA (MSG)");
...
static void __ClessSample_PayPass_DumpRrpProcessingTime (unsigned char * pRrpProcessingTimeValue, unsigned long ulRrpProcessingTimeLength)
{
	...
	if (ClessSample_DumpData_DumpOpenOutputDriver())
	{
		...
		ClessSample_DumpData_DumpNewLine();
		ClessSample_DumpData ("RRP PROCESSING TIME");
		ClessSample_DumpData_DumpNewLine();
		ClessSample_DumpData ("Decimal: %08lu", ulRrpTime);
		ClessSample_DumpData_DumpNewLine();
		ClessSample_DumpData ("Hex: %08lX", ulRrpTime);
		ClessSample_DumpData_DumpNewLine();
		ClessSample_DumpData ("###################");
		ClessSample_DumpData_DumpNewLine();
		ClessSample_DumpData_DumpNewLine();
...
void ClessSample_PayPass_DumpSpecificStructure (T_SHARED_DATA_STRUCT * pExchangeStruct, unsigned long ulTag, int bRequestToKernelIfNotPresent)
{
	...
	if (gs_pTransactionDataLog != NULL)
	{
		if ((ulTag == TAG_PAYPASS_OUTCOME_PARAMETER_SET) || (ulTag == TAG_PAYPASS_ERROR_INDICATION) || (ulTag == TAG_PAYPASS_USER_INTERFACE_REQUEST_DATA) ||
			(ulTag == TAG_PAYPASS_DATA_RECORD) || (ulTag == TAG_PAYPASS_DISCRETIONARY_DATA) || (ulTag == TAG_PAYPASS_RRP_MEASURED_PROCESSING_TIME))
		{
		...
		do
		{
			...
			if (nResult == STATUS_SHARED_EXCHANGE_OK)
			{
				...
				case (TAG_PAYPASS_USER_INTERFACE_REQUEST_DATA):
					...
					__ClessSample_PayPass_DumpUird (pReadValue, ulReadLength, bIsOutUird);
					break;
				...
				case (TAG_PAYPASS_RRP_MEASURED_PROCESSING_TIME):
					__ClessSample_PayPass_DumpRrpProcessingTime (pReadValue, ulReadLength);
					break;
				default:
					break;
				}
...
//		APEMV_UI_MessageDisplayDelay("pCvm",2);
//		APEMV_UI_MessageDisplayDelayHexa(pCvm,1,2);
	}
...
static int __ClessSample_PayPass_AddPayPassSpecificData (T_SHARED_DATA_STRUCT * pDataStruct)
{
	...
	// stouargui add reglage Pb 9F1D le 030918


	T_TI_TAG_INFO propTags[] =
	{
	{0x9F1D, 8, 8, {TAG_SOURCE_TERMINAL, TAG_FORMAT_B, LENGTH_RANGE, OCCURENCE_SINGLE,
	TI_OPTIONS_RFU_MASK_EMPTY_TAG_ALLOWED}, 0, INDEX_NONE},
	};

//	const unsigned char pNewDataObjectValue =0b01000100;
	const unsigned char pNewDataObjectValue =0b01000100;

	// stouargui add reglage Pb 9F1D le 030918 fin
...
/*
	cr = GTL_SharedExchange_AddTag (pDataStruct, TAG_KERNEL_PROPRIETARY_TAGS, sizeof(propTags),propTags);
	if(cr != STATUS_SHARED_EXCHANGE_OK)
	{
	// An error occurred

	}
	cr = GTL_SharedExchange_AddTag (pDataStruct, 0x9F1D, 8, &pNewDataObjectValue);
	if(cr != STATUS_SHARED_EXCHANGE_OK)
	{
	// An erroroccurred
	// ...
...
*/
	...
	//ADD_STEP_CUSTOM(STEP_PAYPASS_CANCEL_DE_PROCESSING, StepCustom); // To allow the application to cancel outstanding DE processing//omoubarak 241121
	...
	//omoubarak 241121
	ADD_STEP_CUSTOM(STEP_PAYPASS_MSTRIPE_SEND_PHONE_MSG, sTransactionFlowCustom.pucStepCustom); // To do GUI when MStripe card has been read
	ADD_STEP_CUSTOM(STEP_PAYPASS_MCHIP_SEND_PHONE_MSG, sTransactionFlowCustom.pucStepCustom); // To do GUI when MChip card has been read
	...
	if (CS_GlobalTransactionData_GetTransactionType(&ucTransactionType))
	{
		...
		if ((ucTransactionType == CLESS_SAMPLE_TRANSACTION_TYPE_MANUAL_CASH) ||
...
int ClessSample_PayPass_CustomiseStep (T_SHARED_DATA_STRUCT * pSharedData, const unsigned char ucCustomisationStep)
{
	...
	case (STEP_PAYPASS_MCHIP_GET_CERTIFICATE):
		...
		//APEMV_UI_MessageDisplayDelayHexa(&ucCapkIndex,1,2);
	...
	case (STEP_PAYPASS_MSTRIPE_SEND_PHONE_MSG):
	   ...
	   //omoubarak 241121
	    PayPass3_GetAllData(pSharedData);
		if (ClessSample_Menu_IsTransactionDataDumpingOn() && (!ClessSample_Menu_IsTransactionLoopOn()))
		//omoubarak 241121
		ClessSample_PayPass_DumpSpecificStructure (pSharedData, TAG_PAYPASS_USER_INTERFACE_REQUEST_DATA, TRUE);
...
static int __ClessSample_PayPass_DisplayBalanceAmount(T_SHARED_DATA_STRUCT * pResultDataStruct)
{
	...
	if(pReadValue[PAYPASS_UIRD_VALUE_QUALIFIER_BYTE] == PAYPASS_UIRD_VALUE_QUALIFIER_BALANCE)
	{
		...
		ClessSample_Term_Read_Message(STD_MESS_AVAILABLE, ClessSample_GuiState_GetCardholderLanguage(), &tDisplayMsgL1);
...
int ClessSample_PayPass_PerformTransaction (T_SHARED_DATA_STRUCT * pDataStruct)
{
	...
	T_SHARED_DATA_STRUCT * PhonesavingDoubleTap;//omoubarak 241121
	uint8   	Status = OK;
	uchar   	l_retour;
	...
	//omoubarak 241121
	T_PAYPASS_PHONE_MESSAGE_TABLE_ENTRY MyOwnTable[3];


	//omoubarak 241121
	MyOwnTable[0].ucPciiMask[0]=0x00; MyOwnTable[0].ucPciiMask[1]=0x08;
	MyOwnTable[0].ucPciiMask[2]=0x00; // PCII MASK = '000800'
	MyOwnTable[0].ucPciiValue[0]=0x00; MyOwnTable[0].ucPciiValue[1]=0x08;
	MyOwnTable[0].ucPciiValue[2]=0x00;// PCIIVALUE = '000800'
	MyOwnTable[0].ucMessage = PAYPASS_UIRD_MESSAGE_ID_SEE_PHONE; // MESSAGE = SEE PHONE
	MyOwnTable[0].ucStatus = PAYPASS_UIRD_STATUS_NOT_READY; // STATUS = NOT READY// Fill the table, item 2
	MyOwnTable[1].ucPciiMask[0]=0x00; MyOwnTable[1].ucPciiMask[1]=0x04; MyOwnTable[1].ucPciiMask[2]=0x00; // PCII MASK = '000400'
	MyOwnTable[1].ucPciiValue[0]=0x00; MyOwnTable[1].ucPciiValue[1]=0x04; MyOwnTable[1].ucPciiValue[2]=0x00; // PCII VALUE = '000400'
	MyOwnTable[1].ucMessage = PAYPASS_UIRD_MESSAGE_ID_APPROVED; // MESSAGE = DECLINED
	MyOwnTable[1].ucStatus = PAYPASS_UIRD_STATUS_NOT_READY; // STATUS = NOT READY// Fill the table, item 3
	MyOwnTable[2].ucPciiMask[0]=0x00; MyOwnTable[2].ucPciiMask[1]=0x00; MyOwnTable[2].ucPciiMask[2]=0x00; // PCII MASK = '000000'
	MyOwnTable[2].ucPciiValue[0]=0x00; MyOwnTable[2].ucPciiValue[1]=0x00; MyOwnTable[2].ucPciiValue[2]=0x00; // PCII VALUE = '000000'
	MyOwnTable[2].ucMessage = PAYPASS_UIRD_MESSAGE_ID_APPROVED; // MESSAGE = DECLINED
	MyOwnTable[2].ucStatus = PAYPASS_UIRD_STATUS_READY_TO_READ; // STATUS = NOT READY// ...// Add the tag in the structure to be used to call PayPass3_DoTransaction() function// We consider that pDataStruct is the shared buffer used to send data to the PayPass kernel
	cr = GTL_SharedExchange_AddTag (pDataStruct, TAG_PAYPASS_INT_PHONE_MESSAGE_TABLE, sizeof(MyOwnTable), (unsigned char *)MyOwnTable);
	if (cr != STATUS_SHARED_EXCHANGE_OK){
		MessageWithDelay("Cannot inject Phone Message Table",NULL, 1000);
	}

	perflog("MG\tPP_CUST\tClessSample_PayPass_PerformTransaction");
	...
	perflog("MG\tPP_CUST\t__ClessSample_PayPass_AddPayPassSpecificData");
	// Get the PayPass
	...
	{	
		...
		///ClessSample_DumpData_DumpSharedBufferWithTitle(pDataStruct, "KERNEL INPUT DATA");
		...
		//omoubarak 241121
		PhonesavingDoubleTap = GTL_SharedExchange_InitShared(SAMPLE_PAYPASS_GET_ALL_DATA_SIZE);
        PayPass3_GetAllData(PhonesavingDoubleTap);
		...
		if (ClessSample_Menu_IsTransactionDataDumpingOn() && (!ClessSample_Menu_IsTransactionLoopOn()))
		{
			...
			ClessSample_PayPass_DumpSpecificStructure (pDataStruct, TAG_PAYPASS_RRP_MEASURED_PROCESSING_TIME, TRUE);
		}
		...
		case (PAYPASS_OPS_STATUS_APPROVED):
			...
			if (!bLoopMode)
			{
				...
				if (pGetAllDataStruct != NULL)
				{
					...
					if ((ClessSample_DumpData_DumpGetOutputId() != CUSTOM_OUTPUT_NONE) && ((bReceiptRequired) || (ucCvm == PAYPASS_OPS_CVM_SIGNATURE)))
					{
						ClessSample_Receipt_PrintPayPass (pGetAllDataStruct, bMobileTransaction, (unsigned int)ucCvm, nCardHolderLang);//KamalCless
					}
...
/*			if (ucCvm == PAYPASS_OPS_CVM_SIGNATURE)
		...
		case (PAYPASS_OPS_STATUS_DECLINED):
			...
			//omoubarak 241121
			__ClessSample_PayPass_DisplayUirdMsg (pDataStruct, TRUE);
...
/*
#define PAYPASS_OPS_CVM_NO_CVM										0x00
#define PAYPASS_OPS_CVM_SIGNATURE									0x10
#define PAYPASS_OPS_CVM_ONLINE_PIN									0x20
#define PAYPASS_OPS_CVM_CONFIRMATION_CODE_VERIFIED					0x30
#define PAYPASS_OPS_CVM_NA											0xF0
*/

			if (pOnlineAuthorisationData != NULL)
			{
				// Check if the required CVM is Online PIN
/*				if (ucCvm == PAYPASS_OPS_CVM_NA)
				{
					APEMV_UI_MessageDisplayDelay ("CVM NOT APPLICABLE", 1);
				}
				else if (ucCvm == PAYPASS_OPS_CVM_NO_CVM)
				{
					APEMV_UI_MessageDisplayDelay ("NO CVM", 1);
				}
				else if (ucCvm == PAYPASS_OPS_CVM_SIGNATURE)
				{
					APEMV_UI_MessageDisplayDelay ("SIGNATURE", 1);
				}
				else if (ucCvm == PAYPASS_OPS_CVM_CONFIRMATION_CODE_VERIFIED)
				{
					APEMV_UI_MessageDisplayDelay ("CONFIRMATION CODE VERIFIED", 1);
				}
				else */
				//if (ucCvm == PAYPASS_OPS_CVM_ONLINE_PIN)
				if(StructInfoPayment.Transaction_Amount>60000)//Version 07.28 600dh au lieu de 400,400dh au lieu de 200
				{
					APEMV_UI_MessageDisplayDelay ("PIN ONLINE", 1);
					...
					if(GetField_pin(153,pin_clair,&Lg_pin) != OK)
						bContinueWithOnlineAuthorisation = B_FALSE;
					else
						bContinueWithOnlineAuthorisation = B_TRUE;
...
/*
				...
				{
					APEMV_UI_MessageDisplayDelay ("NO CVM REQUIRED", 1);
...
*/
				...
				if (bContinueWithOnlineAuthorisation == B_TRUE)
				{
					...
					if (__ClessSample_PayPass_FillBufferForOnlineAuthorisation (pDataStruct, pOnlineAuthorisationData))
					{
						...
						Remplir_Infopayment_Cless (pOnlineAuthorisationData, nCardHolderLang);
						iRet = valide_carte();//badr


						if(iRet == TRANS_FILE_FULL)/*copie famri *///Version 07.28
							{
								memcpy(R_Transac8583.champs39,"305",3);
								Retour_Caisse(R_Transac8583.champs39);			//Cless_ Trs Fichier verouillé
							}
						if(iRet != OK)
							{
						 		//APEMV_UI_MessageDisplayDelay ("", 1);
								ShowTrace("TC2");
...
//						MessageWithDelay("demautor", NULL, 1000);
						l_retour = DemAutor8583(update_param,update_date);//KamalCLess
							...
							//if (GTL_SharedExchange_FindNext(pOnlineAuthorisationData, &nPosition, TAG_EMV_AUTHORISATION_RESPONSE_CODE, &ulReadLength, &pReadValue) == STATUS_SHARED_EXCHANGE_OK)
							{
								...
								if(!memcmp(R_Transac8583.champs39,"000",3))
								{
									...
									if (!bLoopMode)
									{
										...
										//MessageWithDelay("ssssssssssss", NULL, 1000);
								...
								else
								{
									...
									//Os__xprinta("Here 6");
									Retour_Caisse(R_Transac8583.champs39);			//Cless_ Trs Non autorisé
								}
							}
						/*	else
							{
