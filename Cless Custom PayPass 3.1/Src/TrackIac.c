...
#ifdef __TELIUM3__
#define I_EMVTOOL_Type_Appli	(0x57ab)
#else
#define I_EMVTOOL_Type_Appli	(0x61)
#endif
...
static int __TrackIAC_ServiceCall(unsigned short usServiceID, StructProcessStep* pProcessStep)
{
	...
	if (Telium_ServiceGet(I_EMVTOOL_Type_Appli, usServiceID, &ucPriority) == 0)
	{
		// Call the service
		if (Telium_ServiceCall(I_EMVTOOL_Type_Appli, usServiceID, sizeof(StructProcessStep), pProcessStep, &nResult) == 0)
		{
			return TRUE;
		}
		else
