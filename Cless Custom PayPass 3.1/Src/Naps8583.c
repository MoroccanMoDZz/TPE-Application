...
uchar	bmp1200[]	        =	{	114,	52	,	133	,	65,  8,	224,	136,	7	};
...
//	else
	{
		res = CallPerformConnectionGPRS_TLS((char*) FicPara.ip_gprs,
				atoi((char*) FicPara.port_gprs));
		if (res == OK)
			return OK ;
		else
			return KO ;
...
//			ShowTrace("F1");
			...
			{
				memcpy(E_Transac8583.champs48,"P10048",6);
			...
			{
				memcpy(E_Transac8583.champs48,"P92003",6);
				E_Transac8583.l_champs48 += 6;
...
/****************************************************************************/
void Code_PointService(uchar *Code_pt_service,uchar Entry_Mode,uchar Entry_Pin,uchar Lg_pin)
{
	uchar	lg_pin;
	lg_pin=Lg_pin;
	...
	Code_pt_service[7]='5' ;
	Code_pt_service[8]='4';
	Code_pt_service[0]='4' ;	//KamalCless
	switch(Entry_Mode)
	{
   	case TRS_MAG :
		//Code_pt_service[0]='2' ;
	...
	case TRS_SCT :
		//Code_pt_service[0]='9' ;
		Code_pt_service[6]='7' ;	//KamalCless

		if(StructInfoPayment.indexClessMag)
			Code_pt_service[6]='3' ;	//KamalCless
		break ;
	}
...
uchar	Fin_jour(uchar Numeroter,uchar InitModem)
{
	...
	Ushort			Essai_cnx = 0;
	Ushort			nbr_tra,nbr_pre,i,j,k,m,nbr_tra_a_envoyer;
	...
	while(i<nbr_tra)
	{
		...
		if(
			(memcmp(transac.Message_Type,"1200",4) != 0) && //CCD
			(!transac.avis)&&(!transac.Flag_Dem_Aut) &&
			((transac.CodeFunction==VOIDCODE) || ((transac.CodeFunction!=VOIDCODE)&&(!transac.StatusAnnulAdjust)))
		  )
		{
			Tab_avis[j]=i;
...
uchar tel_auto(void)
{
	...
	if(FicPara.status==KO)
		Os__xprint("Auto Job:Para status KO");
	...
	if ( (FicPara.Nbr_of_redial== '0')||(FicPara.compteur_appel >= FicPara.Nbr_of_redial) );
		//Os__xprint("Auto Job:compt app>=Nbr re dial");
	else
	{
		Telium_Read_date(&date);
