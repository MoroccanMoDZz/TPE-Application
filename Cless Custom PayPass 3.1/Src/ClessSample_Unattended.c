...
int ClessSample_Unattended_IsUnattendedMechanism (void)
{
	...
	if ((nInternalDeviceType == TERM_MR40) ||
		(nInternalDeviceType == TERM_IUI120) ||
		(nInternalDeviceType == TERM_IUN180) ||
		(nInternalDeviceType == TERM_IUC180B) ||
		(nInternalDeviceType == TERM_IUN150) ||
		(nInternalDeviceType == TERM_IUN230) ||
		(nInternalDeviceType == TERM_IST150) ||
		(nInternalDeviceType == TERM_IUC285))
	{
		nResult = TRUE;
	}
...
/*	nInternelDeviceType = GetTerminalType();
...
int ClessSample_Unattended_IsUnattendedTerminalWithDisplayWithUCMC (void)
{
	...
	if (PSQ_Is_Ucm_Behaviour())
	{
		...
		if ((nInternalDeviceType == TERM_IUN230) ||
			(nInternalDeviceType == TERM_IUI120) ||
			(nInternalDeviceType == TERM_IUC285))
		{
			nResult = TRUE;
...
void ClessSample_Unattended_ExecuteCommand (int nCommandLength, char * pCommand)
{
	...
	if (nCommandLength > 0)
	{
		...
		else if ((nCommandLength >= 3) && (memcmp(pCommand, "spe", 3) == 0))
		{
			...
			ClessSample_ExplicitSelection_Process (TRUE, FALSE);
		...
		else // Keep value
		{
			...
			case (TXN_START_TTYPE_ENTRY):
				...
				case 5:
					...
					break;
				case 6:
					gs_tUnattendedTransactionData.nTransactionType = CLESS_SAMPLE_TRANSACTION_TYPE_CASH_DISBURSEMENT;
					ClessSample_Term_Read_Message(STD_MESS_MENU_MAIN_CASH_DISBURSMENT, PSQ_Give_Language(), &tDisplayMsg);
					ClessSample_Unattended_PrintDump(tDisplayMsg.message);
					bStartTransaction = TRUE;
					gs_tUnattendedTransactionData.ulAmountOther = 0;
					break;
			...
			if (bStartTransaction)
			{
				if (gs_tUnattendedTransactionData.bExplicitSelection)
				{
					ClessSample_Comevent_Manage (DO_CLOSE);
					// Start the transaction
					ClessSample_ExplicitSelection_Process (FALSE, FALSE);
					ClessSample_Comevent_Manage (DO_OPEN);
...
int ClessSample_Unattended_StartIamTask(void)
{
	...
	// Launch the task
	TaskHdl1 = Telium_Fork(__ClessSample_Unattended_IamTask,NULL,0);

	// Get the task number
	num_task = Telium_GiveNoTask (TaskHdl1);

	GTL_Traces_TraceDebug("start_iam_task %x",Telium_GiveNoTask(TaskHdl1));
...
static word __ClessSample_Unatteded_StartCancelTask (void)
{
	...
	g_ScanningCancelTask = Telium_CurrentTask(); // get the Scanning task ID and store it in a global variable
	...
	while (!g_bScanningCancel_task_to_be_killed) // While the task is not to be killed by the custom application
	{
		if (bWaitToBeKilled) // A cancel has been sent to the Engine, task is waiting to be killed
		{
			if (hCOM0)
			{
				Telium_Fclose (hCOM0); // Close the keyboard driver
				hCOM0 = NULL;
			}
		...
		if (bStartIndication)
		{
			...
			// Allows to 
			if (Telium_SignalEvent (g_wCurrentCancelTask, (tEvent)E_USER_EVENT_START) != cOK)
				GTL_Traces_DiagnosticText("Cancel task 2: Error when setting the event\n");
		}
		
		nEvent = Telium_Ttestall (COM0 | USER_EVENT, 1); // g_ListOfEvent contains the list of other cancel events (here it is CAM0 and SWIPE)
		...
		if (bSendCancel)
		{
			...
			if (ClessSample_Unattended_IsUnattendedMechanism())
				if (ClessSample_Unattended_IsUCMCpresent())
					if (Telium_SignalEvent (g_ScanningTask, (tEvent)E_USER_EVENT_CANCEL_UNATTENDED_UCMC) != cOK)
	...
	if (hCOM0)
		Telium_Fclose (hCOM0); // Close the keyboard
	...
	if (Telium_SignalEvent (g_wCurrentCancelTask, (tEvent)E_USER_EVENT) != cOK)
...
int ClessSample_Unattended_LaunchCancelTask (void)
{
	...
	g_wCurrentCancelTask = Telium_CurrentTask();

	// Launch the scanning task
	g_tsScanningCancel_task_handle = Telium_Fork (&__ClessSample_Unatteded_StartCancelTask, NULL, -1);

	// The task cannot be created
	if (g_tsScanningCancel_task_handle == NULL)
		return KO;
	
	g_ScanningCancelTask = Telium_GiveNoTask (g_tsScanningCancel_task_handle);
...
void ClessSample_Unattended_KillCancelTask (void)
{
	if (g_tsScanningCancel_task_handle != NULL) // If the task is lunched
	{
		...
		if (Telium_SignalEvent (g_ScanningCancelTask, (tEvent)E_USER_EVENT) != cOK)
		...
		//Telium_Kill(g_tsScanningCancel_task_handle, "-*");
...
static void __ClessSample_Unattended_InitCancelVariables (void)
{
	...
	g_ScanningCancelTask = Telium_CurrentTask(); // Init the scanning task number
