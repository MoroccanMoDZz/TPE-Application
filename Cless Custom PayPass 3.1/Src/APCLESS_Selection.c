...
int APCLESS_Selection_GiveInfo (TLV_TREE_NODE * tlvTree, const int explicitSelection)
{
   ...
   // PayPass AID2 parameters
   unsigned char aidPaypass2[] 										= {0xA0, 0x00, 0x00, 0x00, 0x04, 0x30, 0x60};
   unsigned char aidPaypassId2[] 									= {0x00, 0x00, 0x00, 0x00};          					// SEPC PayPass AID internal identifier = 0
   unsigned char aidPaypassKernelToUse2[] 							= {0x00, 0x02};             							// PayPass kernel number = 2
   unsigned char aidPaypassOptions2[] 								= {0x45, 0x01, 0x00, 0x00};     						// Partial AID, Zero Amount, PPSE method
   ...
   // payWave AID2 parameters
   unsigned char aidpayWave2[] 					 					= {0xA0, 0x00, 0x00, 0x00, 0x03, 0x20, 0x10};
   unsigned char aidpayWaveId2[] 				 					= {0x00, 0x00, 0x00, 0x01};          					// SEPC payWave AID internal identifier = 1
   unsigned char aidpayWaveKernelToUse2[] 		 					= {0x00, 0x03};             							// Visa payWave kernel number = 3
   unsigned char aidpayWaveOptions2[] 			 					= {0x01, 0x01, 0x00, 0x00};     						// Partial AID, PPSE method
   unsigned char aidpayWaveTTQ2[] 				 					= {0xB6, 0x00, 0xC0, 0x00};

   // payWave AID2 parameters
   unsigned char aidpayWave3[] 					 					= {0xA0, 0x00, 0x00, 0x00, 0x03, 0x30, 0x10};
   unsigned char aidpayWaveId3[] 				 					= {0x00, 0x00, 0x00, 0x01};          					// SEPC payWave AID internal identifier = 1
   unsigned char aidpayWaveKernelToUse3[] 		 					= {0x00, 0x03};             							// Visa payWave kernel number = 3
   unsigned char aidpayWaveOptions3[] 			 					= {0x01, 0x01, 0x00, 0x00};     						// Partial AID, PPSE method
   unsigned char aidpayWaveTTQ3[] 				 					= {0xB6, 0x00, 0xC0, 0x00};
   ...
   if (tlvTree != NULL)
   {
      ...
      // TAG_AID_TERMINAL : 0x9F06  -> MasterCard PayPass AID
      VERIFY(TlvTree_AddChild(tmpAidInfo, TAG_EMV_AID_TERMINAL, aidPaypass2, sizeof(aidPaypass2)) != NULL);

      // TAG_GENERIC_AID_PROPRIETARY_IDENTIFIER : 0x9F928202 (corresponds to the index for quick search after application selection)
      VERIFY(TlvTree_AddChild(tmpAidInfo, TAG_GENERIC_AID_PROPRIETARY_IDENTIFIER, aidPaypassId2, sizeof(aidPaypassId2)) != NULL);

      // TAG_EP_KERNEL_TO_USE : 0x9F928101
      VERIFY(TlvTree_AddChild(tmpAidInfo, TAG_EP_KERNEL_TO_USE, aidPaypassKernelToUse2, sizeof(aidPaypassKernelToUse2)));

      // TAG_EP_AID_OPTIONS : 0x9F928100
      VERIFY(TlvTree_AddChild(tmpAidInfo, TAG_EP_AID_OPTIONS, aidPaypassOptions2, sizeof(aidPaypassOptions2)));
      ...
      // TAG_AID_TERMINAL : 0x9F06  -> Visa payWave AID
      VERIFY(TlvTree_AddChild(tmpAidInfo, TAG_EMV_AID_TERMINAL, aidpayWave2, sizeof(aidpayWave2)) != NULL);

      // TAG_GENERIC_AID_PROPRIETARY_IDENTIFIER : 0x9F928202 (corresponds to the index for quick search after application selection)
      VERIFY(TlvTree_AddChild(tmpAidInfo, TAG_GENERIC_AID_PROPRIETARY_IDENTIFIER, aidpayWaveId2, sizeof(aidpayWaveId2)) != NULL);

      // TAG_EP_KERNEL_TO_USE : 0x9F928101
      VERIFY(TlvTree_AddChild(tmpAidInfo, TAG_EP_KERNEL_TO_USE, aidpayWaveKernelToUse2, sizeof(aidpayWaveKernelToUse2)));

      // TAG_EP_AID_OPTIONS : 0x9F928100
      VERIFY(TlvTree_AddChild(tmpAidInfo, TAG_EP_AID_OPTIONS, aidpayWaveOptions2, sizeof(aidpayWaveOptions2)));

      // TAG_EP_TERMINAL_TRANSACTION_QUALIFIERS : 0x9F66
      VERIFY(TlvTree_AddChild(tmpAidInfo, TAG_EP_TERMINAL_TRANSACTION_QUALIFIERS, aidpayWaveTTQ2, sizeof(aidpayWaveTTQ2)));
      ...
      // TAG_AID_TERMINAL : 0x9F06  -> Visa payWave AID
      VERIFY(TlvTree_AddChild(tmpAidInfo, TAG_EMV_AID_TERMINAL, aidpayWave3, sizeof(aidpayWave3)) != NULL);

      // TAG_GENERIC_AID_PROPRIETARY_IDENTIFIER : 0x9F928202 (corresponds to the index for quick search after application selection)
      VERIFY(TlvTree_AddChild(tmpAidInfo, TAG_GENERIC_AID_PROPRIETARY_IDENTIFIER, aidpayWaveId3, sizeof(aidpayWaveId3)) != NULL);

      // TAG_EP_KERNEL_TO_USE : 0x9F928101
      VERIFY(TlvTree_AddChild(tmpAidInfo, TAG_EP_KERNEL_TO_USE, aidpayWaveKernelToUse3, sizeof(aidpayWaveKernelToUse3)));

      // TAG_EP_AID_OPTIONS : 0x9F928100
      VERIFY(TlvTree_AddChild(tmpAidInfo, TAG_EP_AID_OPTIONS, aidpayWaveOptions3, sizeof(aidpayWaveOptions3)));

      // TAG_EP_TERMINAL_TRANSACTION_QUALIFIERS : 0x9F66
      VERIFY(TlvTree_AddChild(tmpAidInfo, TAG_EP_TERMINAL_TRANSACTION_QUALIFIERS, aidpayWaveTTQ3, sizeof(aidpayWaveTTQ3)));
...
/**
 * This function is called by the Entry Point to customize the Grapical User Interface during Selection.
