...
static T_GL_HGRAPHIC_LIB __APCLESS_GuiGoal_libInstance = NULL;  ///< handle on GOAL context
static T_GL_HWIDGET __APCLESS_GuiGoal_window = NULL;            ///< handle to the current window
...
int APCLESS_Gui_TransactionBegin(void)
{
	int result;
	...
	GL_GraphicLib_OpenApplicationWindow(APCLESS_Gui_GoalHandle());

	return result;
}

//! \brief Restore the user interface environment as it was before the transaction.
void APCLESS_Gui_TransactionEnd(int previous)
{
#ifndef __TELIUM3__
	...
	GL_GraphicLib_CloseApplicationWindow(APCLESS_Gui_GoalHandle());
...
static void __APCLESS_GuiGoal_DisplayText(const char* line1, const char* line2, const char* line3, const char* line4)
{
   ...
   __APCLESS_GuiGoal_window = GL_GraphicLib_ReadResource(APCLESS_Gui_GoalHandle(), "dal://APCLESSGuiText");
   ...
   {
      GL_Dialog_Message(APCLESS_Gui_GoalHandle(), NULL, "Graphic resource\nnot found", GL_ICON_ERROR, GL_BUTTON_NONE, 2 * GL_TIME_SECOND);
   }
...
{
	APCLESS_Gui_GoalHandle();
}


T_GL_HGRAPHIC_LIB  APCLESS_Gui_GoalHandle(void)
{
   ...
   return __APCLESS_GuiGoal_libInstance;
}
...
int APCLESS_Gui_Menu( const char* title, int defaultItem, const char* items[])
{
   ...
   if(keyboard != NULL)
   {
      ...
      ret = GL_Dialog_Menu(APCLESS_Gui_GoalHandle(), title, items, defaultItem,
...
void APCLESS_Gui_PresentCard(const char* currencyLabel, unsigned int currencyExp, const unsigned char* amountBcd, unsigned int amountLength)
{
   ...
   __APCLESS_GuiGoal_window = GL_GraphicLib_ReadResource(APCLESS_Gui_GoalHandle(), "dal://APCLESSGuiPresent");
...
void APCLESS_Gui_DisplayScreen(unsigned long screenId)
{
   ...
   switch (screenId)
   {
      case APCLESS_SCREEN_RETRY:
         ...
         MessageWithoutConfirmation("TRY AGAIN",NULL);
      ...
      case APCLESS_SCREEN_ERROR_STATUS:
         ...
         MessageWithoutConfirmation("ERROR STATUS",NULL);
         APCLESS_Gui_IndicatorAlert();
         break;

      case APCLESS_SCREEN_REPRESENT_CARD:
         //__APCLESS_GuiGoal_DisplayText(0, "REPRESENT CARD", 0, 0);
         MessageWithoutConfirmation("REPRESENT CARD",NULL);
         break;
      ...
      case APCLESS_SCREEN_PHONE_INSTRUCTIONS_RETRY:
         ...
         MessageWithoutConfirmation("SEE PHONE FOR \nINSTRUCTIONS",NULL);
         APCLESS_Gui_IndicatorAlert(); //Bip Buzzer
         break;

      case APCLESS_SCREEN_CARD_NOT_SUPPORTED:
         //__APCLESS_GuiGoal_DisplayText(0, "NOT SUPPORTED", 0, 0);
         MessageWithoutConfirmation("NOT SUPPORTED",NULL);
         APCLESS_Gui_IndicatorNotReady();
         break;
      ...
      case APCLESS_PAYPASS_SCREEN_AUTHORISING:
         ...
         MessageWithoutConfirmation("AUTHORISING \nPLEASE WAIT",NULL);
         APCLESS_Gui_IndicatorNotReady();
      break;
      ...
      case APCLESS_PAYPASS_SCREEN_WAIT_CARD_REMOVAL: //rem : Appl sould call CLESS_DeselectCard() after
         //__APCLESS_GuiGoal_DisplayText(0, "PLEASE REMOVE", "CARD OR DEVICE", 0);
    	  MessageWithoutConfirmation("PLEASE REMOVE \nCARD OR DEVICE",NULL);
         APCLESS_Gui_IndicatorNotReady();
         break;
