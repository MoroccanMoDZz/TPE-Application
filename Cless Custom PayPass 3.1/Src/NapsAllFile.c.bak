...
L	GetAllParam()
{
	...
	if (iret == FS_OK)
	{
		...
		if (file1 != NULL)
		{
			...
			memcpy(&StructInfoPayment,&Fic_Parametre.StructInfoPayment,sizeof(infopayment));
...
L	PutAllParam()
{
	...
	if (iret == FS_OK)
	{
		...
		if (file1 != NULL)
		{
			...
			memcpy(&Fic_Parametre.StructInfoPayment		,&StructInfoPayment,sizeof(infopayment));
...
void	Init_Param(void)
{
	...
	memset(&StructInfoPayment,0,sizeof(infopayment));
	...
	Os__saved_copy((PUC)"1234",(uchar *)&FicPara.Merchant_Pwd,6);
...
uchar	Trait_transfer_Key(uchar Premier_Mess)
{
	...
	memcpy(bkey,   "\xD9\xDF\x45\xAE\x8C\x62\x01\x58",8);//Test17
	memcpy(bkey+8, "\x1A\x46\x83\xEA\x7F\x26\x0E\xCE",8);//Test17
	memcpy(bkey+16,"\xD9\xDF\x45\xAE\x8C\x62\x01\x58",8);//Test17
	...
	{
		Os__xprint("Pb check value MasterKey");
		return KO;
	}
...
uchar	Trait_transfer_MacKey(uchar Premier_Mess)
{
	...
	{
		Os__xprint("Pb check value MacKey");
		return KO;
	}
