...
{
		// Do not use the printer on the iDE280 due to Cless and PRINTER contention in this device
...
Telium_File_t * ClessSample_DumpData_DumpGetOutputHandle (void)
{
...
#ifdef __TELIUM2__
	nLength = Telium_Vsprintf(String, pString, &Params);
#else
	nLength = Telium_Vsprintf(String, pString, Params);
#endif
	...
	switch (ClessSample_DumpData_DumpGetOutputId())
	{
		...
		case (CUSTOM_OUTPUT_COM0):
		{
			if (ClessSample_DumpData_DumpGetOutputHandle() != NULL)
			{
				...
				do
				{
					i += Telium_Fwrite(String + i, 1, len - i, ClessSample_DumpData_DumpGetOutputHandle());
					if (i < len)
						Telium_Ttestall(0,1);
				} while(i < len);
				Telium_Ttestall (0,1);
