...
static const Message_t Messages_en[]=
{
	...
	{ STD_MESS_OFFLINE_APPROVED, "APPROVED" },
...
static const Message_t Messages_fr[]=
{
	...
	{ STD_MESS_CARD_NOT_SUPPORTED, "CARTE NON GEREE" },
	{ STD_MESS_MENU_TORN_LOG, "TORN LOG" },
...
{
	int lg, x, y, nInternalDeviceType;
	...
	// Specific code for iCMP where two last lines are very close
	nInternalDeviceType = TM_GetTerminalType();
	y = FontHeight * nline + (((nline == (nNbLineInScreen-2)) && (nInternalDeviceType == TERM_ICM122) && (nFontSize == HELPERS_FONT_SIZE_MEDIUM))*5);
	x = C_FIRSTCOL;
...
void ClessSample_Term_Initialise(void)
{
...
#else
	pLocalMessages = (unsigned char*)ClessSample_lang;
#endif // _SIMULPC_
	...
	DefCurrentLang("fr");//Cless_ Forcer le Francais
	DefDefaultLang("fr");//Cless_ Forcer le Francais
}
...
void ClessSample_Term_Read_Message(int nNum, int nLanguage, MSGinfos * pMsgInfo)
{
	...
	DefCurrentLang("fr");//Cless_ Forcer le Francais	DefCurrentLang(ac_l_iso);
