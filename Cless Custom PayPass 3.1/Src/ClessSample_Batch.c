...
int __ClessSample_Batch_ReadTransactionSeqCounter (void);
...
void ClessSample_Batch_InitTransactionSeqCounter (const unsigned char bForceInit)
{
	...
	{
		__ClessSample_Batch_ReadTransactionSeqCounter ();
...
void ClessSample_Batch_PrintTotals (void)
{
	...
	if (ret == FS_OK)
	{
		...
		if (ClessSample_DumpData_DumpOpenOutputDriver())
		{
			...
			ClessSampleFormatSpendingAmount((unsigned char*)ucTotalsNum, 6, ucAsciiTotals);
			ClessSample_DumpData ("Total = %s", ucAsciiTotals);
	...
	if (ClessSample_DumpData_DumpOpenOutputDriver())
	{
		...
		if (hData != NULL)
		{
			...
			if (nDataLength > 0)
			{
				...
				ClessSampleFormatSpendingAmount (pValue, nDataLength, ucAsciiAmount);
				ClessSample_DumpData ("Amount : %s", ucAsciiAmount);
				ClessSample_DumpData_DumpNewLine();
			}
		}
